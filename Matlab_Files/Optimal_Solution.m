...
% m2html('mfiles','./', 'htmldir','doc', 'recursive','on', 'global','on','download','on','graph','on');
...
% 2 - We compute admissible Optimal Controls. We fix the delay constraint, but we do not penalize the derivative.
% 3 - We do [1] and after [2].
% 4 - We compute admissible Optimal Controls. We fix the delay constraint, and we penalize the derivative.
% 5 - We compute admissible Optimal Controls. We fix the delay constraint, we penalize the derivative, and we make Baygorria a ROR dam.
% 6 - We compute the Optimal Controls but letting the virtual controls to have any value, and also removing the derivatives penalization.
% 7 - We load and use the Historical Generation.
%
% When GradNum = 1000, we save the controls of the optimal path after we compute them.
% When GradNum = 555, we do not save the outputs.
% The controls are [turBon, spillBon, turBay, spillBay, turPal, turSA, Battery, Mot, PTA, PTB, CTR, Failure, V2, V3].
...
%     Lambda32 = Lambda32/1e6; % We pass from USD/hm^3 to USD/m^3.
    ...
    % Remmeber to vectorize everything, i.e., V(v1,v2,...,vn) = V(vec) with vec = [v1,v2,...,vn].
    ...
    display = 'notify';
    display = 'none';
    options = optimoptions('fmincon','Display',display,'MaxFunctionEvaluations',1e4,...
...
%         CostTermicas = CostTermicas/1000;
    ...
    NV1 = 2^(Expan-2*0-1); % Discretizations of Bonete.
    NV2 = 2^(Expan); % Discretizations of Baygorria.
    NV3 = 2^(Expan-2*0-1); % Discretizations of Palmar.
    NV4 = 2^(Expan+1*0); % Discretizations of Salto Grande.
    if UseBattery == 1
    ...
    Inflow1 = Matrix{4}(2)*1e6/TMax; % Flow in m^3/s (Bonete).
    Inflow2 = Matrix{5}(2)*1e6/TMax; % Flow in m^3/s (Baygorria).
    Inflow3 = Matrix{6}(2)*1e6/TMax; % Flow in m^3/s (Palmar).
    Inflow4 = Matrix{7}(1); % Flow in m^3/s (Salto Grande).
    Spill1  = Matrix{4}(3)*1e6/TMax; % Historical Spillage in m^3/s (Bonete).
    Spill2  = Matrix{5}(3)*1e6/TMax; % Historical Spillage in m^3/s (Baygorria).
    Spill3  = Matrix{6}(3)*1e6/TMax; % Historical Spillage in m^3/s (Palmar).
    ...
    if UseLamb21 == 0 || (UseLamb21 == 1 && length(Lambda21) == 1)
        Lambda21 = 0; % In USD/hm^3.
    end
    if UseLamb32 == 0 || (UseLamb32 == 1 && length(Lambda32) == 1)
        Lambda32 = 0; % In USD/hm^3.
    end
    lenLambda21 = length(Lambda21);
    lenLambda32 = length(Lambda32);
    D21         = floor((T-Z21)/lenLambda21); % Major discretization for Lambda 21.
    D32         = floor((T-Z32)/lenLambda32); % Major discretization for Lambda 32.
    auxLambda21 = Lambda21*1e-6; % We pass from USD/hm^3 to USD/m^3.
    auxLambda32 = Lambda32*1e-6; % We pass from USD/hm^3 to USD/m^3.
    Lambda21    = zeros(1,length(time)+max(Z21,Z32)); % We add zeros for times > T.
    Lambda32    = zeros(1,length(time)+max(Z21,Z32)); % We add zeros for times > T.
    % Lambda21 and Lambda32 in USD/hm^3. After the next FOR, they will be
    % in USD/m^3 and ready to be plot and used.
    for i = 0:lenLambda21-1
        ...
        lambda21ChangingPosition(i+1) = Z21+i*D21;
        lambda21ChangingValue(i+1) = auxLambda21(i+1) * 1e6;
    end
    for i = 0:lenLambda32-1
        ...
        lambda32ChangingPosition(i+1) = Z32+i*D32;
        lambda32ChangingValue(i+1) = auxLambda32(i+1) * 1e6;
    end
    ...
    if ComputePlots == 1
        ...
        xlim([0 max(lamTime)]);
        for i = 1:lenLambda21
            text(lamTime(lambda21ChangingPosition(i)),lambda21ChangingValue(i),...
                num2str(lambda21ChangingValue(i)),'Color','blue','FontSize',14);
        end
        for i = 1:lenLambda32
            text(lamTime(lambda32ChangingPosition(i)),lambda32ChangingValue(i),...
                num2str(lambda32ChangingValue(i)),'Color',[1 0.25 0],'FontSize',14);
        end
    ...
    % NEW!!!:
    maxWaterChange1 = max(IT1(1),MaxTot1);
    maxWaterChange2 = max(IT2(1),MaxTot2);
    maxWaterChange3 = max(IT3(1),RealMaxFlow3);
    maxWaterChange4 = max(IT4(1),RealMaxFlow4);
    Vmax1 = min((Vini1*VolMax1+(maxWaterChange1)*TMax*expFactor),VolMax1)/VolMax1;
    Vmin1 = max((Vini1*VolMax1-(maxWaterChange1)*TMax*expFactor),VolMin1)/VolMax1;
    Vmax2 = min((Vini2*VolMax2+(maxWaterChange2)*TMax*expFactor),VolMax2)/VolMax2;
    Vmin2 = max((Vini2*VolMax2-(maxWaterChange2)*TMax*expFactor),VolMin2)/VolMax2;
    Vmax3 = min((Vini3*VolMax3+(maxWaterChange3)*TMax*expFactor),VolMax3)/VolMax3;
    Vmin3 = max((Vini3*VolMax3-(maxWaterChange3)*TMax*expFactor),VolMin3)/VolMax3;
    Vmax4 = min((Vini4*VolMax4+(maxWaterChange4)*TMax*expFactor),VolMax4)/VolMax4;
    Vmin4 = max((Vini4*VolMax4-(maxWaterChange4)*TMax*expFactor),VolMin4)/VolMax4;
...
%     [vplotA_1,vplot1_A] = meshgrid(A,V1); % For plotting surfaces.
    ...
    if (WhatToDo == 1) || (WhatToDo == 6) % Solve HJB.
        Khe1 = Matrix{3}(3); % In USD/MWh. Cost per energy of Bonete.
        Khe2 = Matrix{3}(4); % In USD/MWh. Cost per energy of Baygorria.
        ...
        Khe2 = max(Matrix{3}(4),minWater2); % In USD/MWh. Cost per energy of Baygorria.
...
%         Khe1 = 0; Khe2 = 0; Khe3 = 0; Khe4 = 0;
    ...
    % The equation is k1 = k2*con1 + k3*con2 or k1 - k2*con1 - k3*con2 = 0.
    
    k1 = @(t,vir1)  (IT2(t) + RealMaxFlow1*vir1) / MaxV2;
    k2 = @(V2,V3)   FlowMax2(V2,V3) / MaxV2;
    k3 = @(V2,V3)   MaxSpill2(V2,V3) / MaxV2;
    k  = @(t,V2,V3,vir1) [k1(t,vir1),k2(V2,V3),k3(V2,V3)];
    ...
    disp(['KF1 = ',num2str(CostTermicas(1)),', KF2 = ',num2str(CostTermicas(2)),', KF3 = ',...
        num2str(CostTermicas(3)),', KF4 = ',num2str(CostTermicas(4)),', KF5 = ',num2str(CostTermicas(5))]);
    disp(['CH1 = ',num2str(CH1),', CH2 = ',num2str(CH2),', CH3 = ',num2str(CH3),', CH4 = ',num2str(CH4)]);
    ...
    if WhatToDo == 1 || WhatToDo == 3
        ...
        for t = length(time):-1:1
            ...
            parfor index = 1:TL
                ...
                % NEW!!!:
                [controls,neutalData] = Quad_FMC_Lambda_8(Q(V1(ind_V1),V2(ind_V2),V3(ind_V3),t),b(V1(ind_V1),V2(ind_V2),V3(ind_V3),...
                        V4(ind_V4),t),c(t),d(V1(ind_V1),V2(ind_V2),V3(ind_V3),uV1_PF(index),v2_aux,v3_aux,uV4_PF(index),...
                        Aux_uA,t),dd,options,WS,Aux_Lims,InfTermicas);
                controls = [controls;Aux_V2;Aux_V3];
                Controls_Aux(index,:) = controls;
                forNeuralNetwork{t,index} = neutalData;
...
%             end
        ...
        save([pwd '/','Simulations/Simulation_',name,'.mat'],'ToSave','-v7.3');
        save([pwd '/','Simulations/dataNN_',name,'.mat'],'forNeuralNetwork','-v7.3');
        toc
    ...
    if WhatToDo == 2 || WhatToDo == 3 || WhatToDo == 4 || WhatToDo == 5 || WhatToDo == 6 || WhatToDo == 7
        ...
        finalCostHJB = Final_Cost/NormCost;
        if WhatToDo ~= 7
        ...
        for t = 1:length(time)
            ...
            if t == 1
                ...
                if UseBattery == 1
                    BA = 1;
                    realxA = (Battery_Real(t)-A(IPA))/(A(IPA+BA)-A(IPA));
                else
                    % This fix well the state of the battery in A=A_0.
                    realxA = A;
                    BA = 0;
                    IPA = 1;
                end
                ...
                valsV1 = [UV{t}{1}(IP1,IP2,IP3,IP4,IPA) ; UV{t}{1}(IP1+1,IP2,IP3,IP4,IPA)       ; UV{t}{1}(IP1,IP2+1,IP3,IP4,IPA)       ; UV{t}{1}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    UV{t}{1}(IP1,IP2,IP3,IP4,IPA+BA)    ; UV{t}{1}(IP1+1,IP2,IP3,IP4,IPA+BA)    ; UV{t}{1}(IP1,IP2+1,IP3,IP4,IPA+BA)    ; UV{t}{1}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    UV{t}{1}(IP1,IP2,IP3+1,IP4,IPA+BA)  ; UV{t}{1}(IP1+1,IP2,IP3+1,IP4,IPA+BA)  ; UV{t}{1}(IP1,IP2+1,IP3+1,IP4,IPA+BA)  ; UV{t}{1}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    UV{t}{1}(IP1,IP2,IP3,IP4+1,IPA+BA)  ; UV{t}{1}(IP1+1,IP2,IP3,IP4+1,IPA+BA)  ; UV{t}{1}(IP1,IP2+1,IP3,IP4+1,IPA+BA)  ; UV{t}{1}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    UV{t}{1}(IP1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{1}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{1}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); UV{t}{1}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
                ...
                valsV2 = [UV{t}{2}(IP1,IP2,IP3,IP4,IPA); UV{t}{2}(IP1+1,IP2,IP3,IP4,IPA); UV{t}{2}(IP1,IP2+1,IP3,IP4,IPA); UV{t}{2}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    UV{t}{2}(IP1,IP2,IP3,IP4,IPA+BA); UV{t}{2}(IP1+1,IP2,IP3,IP4,IPA+BA); UV{t}{2}(IP1,IP2+1,IP3,IP4,IPA+BA); UV{t}{2}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    UV{t}{2}(IP1,IP2,IP3+1,IP4,IPA+BA); UV{t}{2}(IP1+1,IP2,IP3+1,IP4,IPA+BA); UV{t}{2}(IP1,IP2+1,IP3+1,IP4,IPA+BA); UV{t}{2}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    UV{t}{2}(IP1,IP2,IP3,IP4+1,IPA+BA); UV{t}{2}(IP1+1,IP2,IP3,IP4+1,IPA+BA); UV{t}{2}(IP1,IP2+1,IP3,IP4+1,IPA+BA); UV{t}{2}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    UV{t}{2}(IP1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{2}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{2}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); UV{t}{2}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
                ...
                valsV3 = [UV{t}{3}(IP1,IP2,IP3,IP4,IPA); UV{t}{3}(IP1+1,IP2,IP3,IP4,IPA); UV{t}{3}(IP1,IP2+1,IP3,IP4,IPA); UV{t}{3}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    UV{t}{3}(IP1,IP2,IP3,IP4,IPA+BA); UV{t}{3}(IP1+1,IP2,IP3,IP4,IPA+BA); UV{t}{3}(IP1,IP2+1,IP3,IP4,IPA+BA); UV{t}{3}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    UV{t}{3}(IP1,IP2,IP3+1,IP4,IPA+BA); UV{t}{3}(IP1+1,IP2,IP3+1,IP4,IPA+BA); UV{t}{3}(IP1,IP2+1,IP3+1,IP4,IPA+BA); UV{t}{3}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    UV{t}{3}(IP1,IP2,IP3,IP4+1,IPA+BA); UV{t}{3}(IP1+1,IP2,IP3,IP4+1,IPA+BA); UV{t}{3}(IP1,IP2+1,IP3,IP4+1,IPA+BA); UV{t}{3}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    UV{t}{3}(IP1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{3}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{3}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); UV{t}{3}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
                ...
                valsV4 = [UV{t}{4}(IP1,IP2,IP3,IP4,IPA); UV{t}{4}(IP1+1,IP2,IP3,IP4,IPA); UV{t}{4}(IP1,IP2+1,IP3,IP4,IPA); UV{t}{4}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    UV{t}{4}(IP1,IP2,IP3,IP4,IPA+BA); UV{t}{4}(IP1+1,IP2,IP3,IP4,IPA+BA); UV{t}{4}(IP1,IP2+1,IP3,IP4,IPA+BA); UV{t}{4}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    UV{t}{4}(IP1,IP2,IP3+1,IP4,IPA+BA); UV{t}{4}(IP1+1,IP2,IP3+1,IP4,IPA+BA); UV{t}{4}(IP1,IP2+1,IP3+1,IP4,IPA+BA); UV{t}{4}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    UV{t}{4}(IP1,IP2,IP3,IP4+1,IPA+BA); UV{t}{4}(IP1+1,IP2,IP3,IP4+1,IPA+BA); UV{t}{4}(IP1,IP2+1,IP3,IP4+1,IPA+BA); UV{t}{4}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    UV{t}{4}(IP1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{4}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{4}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); UV{t}{4}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
                ...
                valsA = [UV{5}{4}(IP1,IP2,IP3,IP4,IPA); UV{t}{5}(IP1+1,IP2,IP3,IP4,IPA); UV{t}{5}(IP1,IP2+1,IP3,IP4,IPA); UV{t}{5}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    UV{t}{5}(IP1,IP2,IP3,IP4,IPA+BA); UV{t}{5}(IP1+1,IP2,IP3,IP4,IPA+BA); UV{t}{5}(IP1,IP2+1,IP3,IP4,IPA+BA); UV{t}{5}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    UV{t}{5}(IP1,IP2,IP3+1,IP4,IPA+BA); UV{t}{5}(IP1+1,IP2,IP3+1,IP4,IPA+BA); UV{t}{5}(IP1,IP2+1,IP3+1,IP4,IPA+BA); UV{t}{5}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    UV{t}{5}(IP1,IP2,IP3,IP4+1,IPA+BA); UV{t}{5}(IP1+1,IP2,IP3,IP4+1,IPA+BA); UV{t}{5}(IP1,IP2+1,IP3,IP4+1,IPA+BA); UV{t}{5}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    UV{t}{5}(IP1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{5}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); UV{t}{5}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); UV{t}{5}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
                ...
                valsU = [U{t}(IP1,IP2,IP3,IP4,IPA); U{t}(IP1+1,IP2,IP3,IP4,IPA); U{t}(IP1,IP2+1,IP3,IP4,IPA); U{t}(IP1+1,IP2+1,IP3,IP4,IPA);...
                    ...
                    U{t}(IP1,IP2,IP3,IP4,IPA+BA); U{t}(IP1+1,IP2,IP3,IP4,IPA+BA); U{t}(IP1,IP2+1,IP3,IP4,IPA+BA); U{t}(IP1+1,IP2+1,IP3,IP4,IPA+BA);...
                    U{t}(IP1,IP2,IP3+1,IP4,IPA+BA); U{t}(IP1+1,IP2,IP3+1,IP4,IPA+BA); U{t}(IP1,IP2+1,IP3+1,IP4,IPA+BA); U{t}(IP1+1,IP2+1,IP3+1,IP4,IPA+BA);...
                    U{t}(IP1,IP2,IP3,IP4+1,IPA+BA); U{t}(IP1+1,IP2,IP3,IP4+1,IPA+BA); U{t}(IP1,IP2+1,IP3,IP4+1,IPA+BA); U{t}(IP1+1,IP2+1,IP3,IP4+1,IPA+BA);...
                    U{t}(IP1,IP2,IP3+1,IP4+1,IPA+BA); U{t}(IP1+1,IP2,IP3+1,IP4+1,IPA+BA); U{t}(IP1,IP2+1,IP3+1,IP4+1,IPA+BA); U{t}(IP1+1,IP2+1,IP3+1,IP4+1,IPA+BA)];
            ...
            if WhatToDo ~= 7
                ...
                if t ~= 1
                    vir1 = Controls_OP(t-1,13);
                else
                    vir1 = 1; % Atention!!! This should be Bonete's spillage and flow from 6 hours ago.
                end

                % NEW!!!:
                if WhatToDo ~= 5
                    [Controls_OP(t,1:12),~] = Quad_FMC_Lambda_10(Q(Vol_OP_Real_1(t),Vol_OP_Real_2(t),Vol_OP_Real_3(t),t),...
                        ...
                        options,x0,ContLims,InfTermicas,k(t,Vol_OP_Real_2(t),Vol_OP_Real_3(t),vir1));
                else
            ...
            elseif WhatToDo == 7
                ...
                % I am assuming no influence from the spillage in the
                % power production. However, the spillage may modify
                % the cost-to-go a lot.
                Controls_OP(t,1) = contBon(histPowerBon(t),0,Vol_OP_Real_1(t),Vol_OP_Real_2(t));
                Controls_OP(t,2) = Spill1/MaxSpill1(Vol_OP_Real_1(t),Vol_OP_Real_2(t));
                Controls_OP(t,3) = contBay(histPowerBay(t),0,Vol_OP_Real_2(t),Vol_OP_Real_3(t));
                Controls_OP(t,4) = Spill2/MaxSpill2(Vol_OP_Real_2(t),Vol_OP_Real_3(t));
            ...
            % problem. (16/10/2019: We added waterIzero with I in {1,2,3,4}).
            % ALERT!!!: I have to add the Lambda.
            realInstantCost(t) = dt*TMax*(sum(Controls_OP(t,:).*...
                [FlowMax1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))*CH1*water1zero,...
                MaxSpill1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))*CH1*water1zero,...
                FlowMax2(Vol_OP_Real_2(t),Vol_OP_Real_3(t))*CH2*water2zero,...
                MaxSpill2(Vol_OP_Real_2(t),Vol_OP_Real_3(t))*CH2*water2zero,...
                FlowMax3(Vol_OP_Real_3(t),t)*CH3*water3zero,...
                FlowMax4*CH4*water4zero,...
                0,... $ Battery.
                CostTermicasNorm.*MaxTermicas,...
                MaxV2*(-CH2)*water2zero,...
                MaxV3*(-CH3)*water3zero]) - ...
                (IT1(t)*CH1*water1zero+IT2(t)*CH2*water2zero...
                +IT3(t)*CH3*water3zero+IT4(t)*CH4*water4zero));
            ...
            % The above vector is:
            % [Bonete,Baygorria,Palmar,SG,MB,PTA,PTB,CTR,Battery,Failure,V2,V3].
...
%                         'Motores Batlle','PTA','PTB','CTR','Failure','Demand','Demand + Exp.','location','eastoutside');
            ...
            xlabel('Time');
            if WhatToDo == 7
                title('Historical Daily Power Balance');
            elseif WhatToDo == 6
                title('Dual Daily Power Balance');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Daily Power Balance');
            elseif WhatToDo == 2
                title('Non-smooth Simulated Daily Power Balance');
            end
            %ylabel('Power (MW)');
            ylabel('Power');
            xlim([0 max(time)]);
            ...
            text(time(floor(length(time)/2)),Final_Cost/NormCost,num2str(Final_Cost/NormCost));
            ...
            if WhatToDo == 7
                title('Historical Accumulated Simulated Cost');
            elseif WhatToDo == 6
                title('Dual Accumulated Simulated Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Accumulated Simulated Cost');
            elseif WhatToDo == 2
                title('Non-smooth Accumulated Simulated Cost');
            end
            xlim([0 max(time)]);
            set(gca,'Box','on');
            ...
            elseif WhatToDo == 6
                title('Dual Dams Controls');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Dams Controls');
            elseif WhatToDo == 2
                title('Non-smooth Dams Controls');
            end
            xlabel('Time');
            ylabel('Control');
            for q = 1:6
            ...
            elseif WhatToDo == 6
                title('Dual Fossil Fuel Stations Controls');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Fossil Fuel Stations Controls');
            elseif WhatToDo == 2
                title('Non-smooth Fossil Fuel Stations Controls');
            end
            xlabel('Time');
            ylabel('Control');
            for q = 8:12
            ...
            if WhatToDo == 7
                title('Historical Battery Control');
            elseif WhatToDo == 6
                title('Dual Battery Control');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Battery Control');
            elseif WhatToDo == 2
                title('Non-smooth Battery Control');
            end
            xlabel('Time'); ylabel('Control');
            plot(time,zeros(1,length(time)),'r');
            legend('Battery'); grid minor;
            ...
            ylabel('Volume');
            if WhatToDo == 7
                title('Historical Volume Bonete');
            elseif WhatToDo == 6
                title('Dual Volume Bonete');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Volume Bonete');
            elseif WhatToDo == 2
                title('Non-smooth Volume Bonete');
            end
            hold on;
            xlim([0 max(time)]);
            for i = 2:length(V1)-1
            ...
            ylabel('Volume');
            if WhatToDo == 7
                title('Historical Volume Palmar');
            elseif WhatToDo == 6
                title('Dual Volume Palmar');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Volume Palmar');
            elseif WhatToDo == 2
                title('Non-smooth Volume Palmar');
            end
            xlim([0 max(time)]);
            for i = 2:length(V3)-1
            ...
            ylabel('Volume');
            if WhatToDo == 7
                title('Historical Volume Salto Grande');
            elseif WhatToDo == 6
                title('Dual Volume Salto Grande');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Volume Salto Grande');
            elseif WhatToDo == 2
                title('Non-smooth Volume Salto Grande');
            end
            xlim([0 max(time)]);
            for i = 2:length(V4)-1
            ...
            ylabel('Capacity');
            if WhatToDo == 7
                title('Historical Battery Capacity');
            elseif WhatToDo == 6
                title('Dual Battery Capacity');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Battery Capacity');
            elseif WhatToDo == 2
                title('Non-smooth Battery Capacity');
            end
            ...
            if WhatToDo == 7
                title('Historical Opportunity Costs');
            elseif WhatToDo == 6
                title('Dual Opportunity Costs');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Opportunity Costs');
            elseif WhatToDo == 2
                title('Non-smooth Opportunity Costs');
            end
            if UseBattery == 1
            ...
            grid on; grid minor;
            if WhatToDo == 7
                title('Historical Battery Opportunity Cost');
            elseif WhatToDo == 6
                title('Dual Battery Opportunity Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Battery Opportunity Cost');
            elseif WhatToDo == 2
                title('Non-smooth Battery Opportunity Cost');
            end
            ...
            grid minor;
            if WhatToDo == 7
                title('Historical FF Opportunity Cost');
            elseif WhatToDo == 6
                title('Dual FF Opportunity Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated FF Opportunity Cost');
            elseif WhatToDo == 2
                title('Non-smooth FF Opportunity Cost');
            end
            if UseBattery == 1
            ...
            grid minor;
            if WhatToDo == 7
                title('Historical Spillage Opportunity Cost');
            elseif WhatToDo == 6
                title('Dual Spillage Opportunity Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Spillage Opportunity Cost');
            elseif WhatToDo == 2
                title('Non-smooth Spillage Opportunity Cost');
            end
            ...
            if WhatToDo == 7
                title('Historical Instant Derivatives Values');
            elseif WhatToDo == 6
                title('Dual Instant Derivatives Values');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Instant Derivatives Values');
            elseif WhatToDo == 2
                title('Non-smooth Instant Derivatives Values');
            end
            xlabel('Time');
            ylabel('Derivatives');
            set(gca,'Box','on');
            ...
            try
                ...
                p1 = pie(X);
                if WhatToDo == 7
                    title('Historical Energy Distribution');
                elseif WhatToDo == 6
                    title('Dual Energy Distribution');
                elseif WhatToDo == 4 || WhatToDo == 5
                    title('Simulated Energy Distribution');
                elseif WhatToDo == 2
                    title('Non-smooth Energy Distribution');
                end
...
%                     title('Controllable Costs Distribution');
                ...
                elseif WhatToDo == 6
                    title('Dual Controllable Costs Distribution');
                elseif WhatToDo == 4 || WhatToDo == 5
                    title('Simulated Controllable Costs Distribution');
                elseif WhatToDo == 2
                    title('Non-smooth Controllable Costs Distribution');
                end
...
%                     legend(combinedtxt,'Location','eastoutside');
            ...
            elseif WhatToDo == 6
                title('Dual Controllable Accumulated Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Controllable Accumulated Cost');
            elseif WhatToDo == 2
                title('Non-smooth Controllable Accumulated Cost');
            end
            ylabel('Hundred Thousand Dollars');
            xlim([0 max(time)]);
            set(gca,'Box','on');
            ...
            xlabel('Time');
            if WhatToDo == 7
                title('Historical Accumulated Cost');
            elseif WhatToDo == 6
                title('Dual Accumulated Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Accumulated Cost');
            elseif WhatToDo == 2
                title('Non-smooth Accumulated Cost');
            end
            ylabel('Hundred Thousand Dollars');
            xlim([0 max(time)]);
            set(gca,'Box','on');
            ...
            elseif WhatToDo == 6
                title('Dual Costs Distribution');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Costs Distribution');
            elseif WhatToDo == 2
                title('Non-smooth Costs Distribution');
            end
            legend(txt,'Location','eastoutside');
            ylabel('Hundred Thousand Dollars');
            set(gca,'Box','on');
            ...
            ylabel('Volume');
            if WhatToDo == 7
                title('Historical Volume Baygorria');
            elseif WhatToDo == 6
                title('Dual Volume Baygorria');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Volume Baygorria');
            elseif WhatToDo == 2
                title('Non-smooth Volume Baygorria');
            end
            xlim([0 max(time)]);
            for i = 2:length(V2)-1
            ...
            if WhatToDo ~= 4 && WhatToDo ~= 6 && WhatToDo ~= 7
                ...
                legend('Spot Price');
                if WhatToDo == 7
                    title('Historical Spot Price');
                elseif WhatToDo == 6
                    title('Dual Spot Price');
                elseif WhatToDo == 4 || WhatToDo == 5
                    title('Simulated Spot Price');
                elseif WhatToDo == 2
                    title('Non-smooth Spot Price');
                end
                xlabel('Time'); ylabel('USD/MWh');
                set(gca,'Box','on');
            end

            set(0,'CurrentFigure',119); clf(119); set(gcf,'name','Error GAP');
            errorGAP = abs(Final_Cost-accumRealInstantCost(end))/abs(Final_Cost);
            A = area(time,ones(1,length(time))*(errorGAP));
            if errorGAP < 3e-2
                A.FaceColor = [0 1 0];
            elseif errorGAP < 5e-2
                A.FaceColor = [1 1 0];
            else
                A.FaceColor = [1 0 0];
            end
            if WhatToDo == 7
                title('Historical Error GAP');
            elseif WhatToDo == 6
                title('Dual Error GAP');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Error GAP');
            elseif WhatToDo == 2
                title('Non-smooth Error GAP');
            end
            set(gca,'Box','on');
...
%                 end
            xlabel('Time');
            if WhatToDo == 7
                title('Historical Daily Power Balance (Controllable)');
            elseif WhatToDo == 6
                title('Dual Daily Power Balance (Controllable)');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Daily Power Balance (Controllable)');
            elseif WhatToDo == 2
                title('Non-smooth Simulated Daily Power Balance (Controllable)');
            end
            ylabel('Power');
            xlim([0 max(time)]);
            ...
            try
                ...
                p2 = pie(X);
                if WhatToDo == 7
                    title('Historical Hydro-Fuel Cost Ratio');
                elseif WhatToDo == 6
                    title('Dual Hydro-Fuel Cost Ratio');
                elseif WhatToDo == 4 || WhatToDo == 5
                    title('Simulated Hydro-Fuel Cost Ratio');
                elseif WhatToDo == 2
                    title('Non-smooth Hydro-Fuel Cost Ratio');
                end
                pText = findobj(p2,'Type','text');
                percentValues = get(pText,'String');
                ax = gca;
            ...
            hold on; grid minor;
            if WhatToDo == 7
                title('Historical Virtual Controls');
            elseif WhatToDo == 6
                title('Dual Virtual Controls');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Virtual Controls');
            elseif WhatToDo == 2
                title('Non-smooth Virtual Controls');
            end
            xlabel('Time');
            ylabel('Control');
            ...
            xlabel('Time'); ylabel('Hundred Thousand Dollars');
            if WhatToDo == 7
                title('Historical Instant Cost');
            elseif WhatToDo == 6
                title('Dual Instant Cost');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Instant Cost');
            elseif WhatToDo == 2
                title('Non-smooth Instant Cost');
            end
            xlim([0 max(time)]);
            set(gca,'Box','on');

            set(0,'CurrentFigure',130); clf(130); set(gcf,'name','Costs in the Optimal Path');
            hold on;
            % olCV2 = @(uV2,t) MaxV2*(Lambda21(t) + uV2/VolMax2 - CH2);
            P = plot(time,CV2InstantCost);                  P.LineWidth = 1;
            P = plot(time,MaxV2*uV2plot/VolMax2);           P.LineWidth = 1;
            P = plot(time,-MaxV2*CH2*ones(1,length(time))); P.LineWidth = 1;
            P = plot(time,MaxV2*Lambda21(1:length(time)));  P.LineWidth = 1;
            grid minor;
            ylabel('USD/Time');
            if WhatToDo == 7
                title('Historical Costs per Time Unit');
            elseif WhatToDo == 6
                title('Dual Costs per Time Unit');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Costs per Time Unit');
            elseif WhatToDo == 2
                title('Non-smooth Costs per Time Unit');
            end
            h = legend('Instant Cost Virtual Baygorria','Partial Derivative $V_2$',...
                'Minus $C_H^{(2)}$','$\lambda_{21}$');
            ...
            P = plot(time,MaxV3*uV3plot/VolMax3);           P.LineWidth = 1;
            P = plot(time,-MaxV3*CH3*ones(1,length(time))); P.LineWidth = 1;
            P = plot(time,MaxV3*Lambda32(1:length(time)));  P.LineWidth = 1;
            grid minor;
            ylabel('USD/Time');
            if WhatToDo == 7
                title('Historical Costs per Time Unit');
            ...
            h = legend('Instant Cost Virtual Palmar','Partial Derivative $V_3$',...
                'Minus $C_H^{(3)}$','$\lambda_{32}$');
            set(h,'Interpreter','Latex');
            set(gca,'Box','on');
            ...
            grid minor;
            if WhatToDo == 7
                title('Historical Water Exchange');
            elseif WhatToDo == 6
                title('Dual Water Exchange');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Water Exchange');
            elseif WhatToDo == 2
                title('Non-smooth Water Exchange');
            end
            ylabel('m^3');
            ...
            grid minor;
            if WhatToDo == 7
                title('Historical Water Balance');
            elseif WhatToDo == 6
                title('Dual Water Balance');
            elseif WhatToDo == 4 || WhatToDo == 5
                title('Simulated Water Balance');
            elseif WhatToDo == 2
                title('Non-smooth Water Balance');
            end
            ylabel('m^3');
            ...
            if SavePlots == 1
                for i = Plot_12:Plot_22
                    ...
                    if WhatToDo == 7
                        if 0 == exist([pwd '/Simulations/Real_',name],'dir')
                            mkdir([pwd '/Simulations/Real_',name]);
                        end
                        saveas(gcf,[pwd '/',['Simulations/Real_',name],'/',num2str(i)],'epsc');
                    elseif WhatToDo == 6
                        if 0 == exist([pwd '/Simulations/Dual_',name],'dir')
                            mkdir([pwd '/Simulations/Dual_',name]);
                        end
                        saveas(gcf,[pwd '/',['Simulations/Dual_',name],'/',num2str(i)],'epsc');
                    elseif WhatToDo == 4
                        if 0 == exist([pwd '/Simulations/Soft_',name],'dir')
                            mkdir([pwd '/Simulations/Soft_',name]);
                        end
                        saveas(gcf,[pwd '/',['Simulations/Soft_',name],'/',num2str(i)],'epsc');
                    elseif WhatToDo == 5
                        if 0 == exist([pwd '/Simulations/BayFixed_',name],'dir')
                            mkdir([pwd '/Simulations/BayFixed_',name]);
                        end
                        saveas(gcf,[pwd '/',['Simulations/BayFixed_',name],'/',num2str(i)],'epsc');
                    else
        ...
        if WhatToDo == 6
            ...
            postPostProcessing = {Final_Cost,UV,CFL_Plot,U};
            save([pwd '/','Simulations/Simulation_',name,'.mat'],'ToSave','-v7.3');
            ...
            Final_Cost = [accumRealInstantCost(end),gradientBaygorria*1e-6,gradientPalmar*1e-6];
    ...
    %                 end
            ...
            if GradNum ~= 555
                ...
                dataFrom_6 = {...
                    finalCostHJB,...
                    accumRealInstantCost(end),...
                    Khe1,Khe2,Khe3,Khe4,...
                    IT1(1),IT2(1),IT3(1),IT4(1)
                    };
                save([pwd '/','Simulations/dataFrom_6_',name,'.mat'],'dataFrom_6');

            end

        elseif WhatToDo == 7 && GradNum ~= 555

            dataFrom_7 = {...
                accumRealInstantCost(end),...
                accumContCost};
            save([pwd '/','Simulations/dataFrom_7_',name,'.mat'],'dataFrom_7');

        elseif WhatToDo == 2 && GradNum ~= 555

            dataFrom_2 = {...
                accumRealInstantCost(end),...
                };
            save([pwd '/','Simulations/dataFrom_2_',name,'.mat'],'dataFrom_2');

        elseif WhatToDo == 4 && GradNum ~= 555

            dataFrom_4 = {...
                accumRealInstantCost(end),...
                accumContCost};
            save([pwd '/','Simulations/dataFrom_4_',name,'.mat'],'dataFrom_4');
            
        elseif WhatToDo == 5 && GradNum ~= 555

            dataFrom_5 = {...
                accumRealInstantCost(end),...
                accumContCost};
            save([pwd '/','Simulations/dataFrom_5_',name,'.mat'],'dataFrom_5');
