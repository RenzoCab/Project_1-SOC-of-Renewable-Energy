function [] = Optimum_Lambda(expansion_T,expansion_S,Day)
    ...
    Op_Lambda = 0;
    DG = 10;
    options = optimset('PlotFcns',@optimplotfval,'TolFun',1e-3,'TolX',1e-6,'MaxFunEvals',200,'MaxIter',200);

    while End ~= 1 && DG > 0.03 && Count ~= 5 % <<<<<----- Remove 'Count ~= 2'.
