function [HJB_Initial_Cost] = Admissible_Solution_6_1W(name,WhatToDo,WarmStart,UseBattery,...
    ComputePlots,NewPlots,SavePlots,Expan,Expan_Time,UseLamb21,UseLamb32,...
    Lambda21,Lambda32,Day)

    Derivatives = [1,1,1,1]; % For debugging.
    
    if 0 == exist(['Simulation_',name],'dir')
        mkdir(['Simulation_',name]);
    end

    load('MaxFlowCoeff.mat');
    load([pwd '/Historical_1W/Day_',num2str(Day),'_2019.mat']);
...
%     MaxTermicas = [Matrix{2}(2),Matrix{2}(4),Matrix{2}(6),Matrix{2}(8),1000]*1000; % kW.
    MaxTermicas = [Matrix{2}(:,4),Matrix{2}(:,6),Matrix{2}(:,8),Matrix{2}(:,2),ones(7,1)*1000]*1000; % kW.
    InfTermicas = [0,0,0,0,0];
%     CostTermicas = [Matrix{2}(1),Matrix{2}(3),Matrix{2}(5),Matrix{2}(7),600]; % USD/MWh.
    CostTermicas = [Matrix{2}(:,3),Matrix{2}(:,5),Matrix{2}(:,7),Matrix{2}(:,1),ones(7,1)*600]; % USD/MWh.
    CostTermicasNorm = CostTermicas/MWhTokW; % USD/kW.

    NT = 7*2^Expan_Time; % Discretizations in time.
    ...
    h04_Vec = Matrix{8}(:,2);
    h03_Vec = Matrix{8}(:,1);
    h04_Vec = interp1(0:1/(length(h04_Vec)-1):1,h04_Vec,0:1/(length(time)-1):1);
    h03_Vec = interp1(0:1/(length(h03_Vec)-1):1,h03_Vec,0:1/(length(time)-1):1);
    h04 = @(t) h04_Vec(t);
    h03 = @(t) h03_Vec(t);
    ...
    IT1 = @(t,d) Inflow1(8-d); % In m^3/s.
    IT2 = @(t,d) Inflow2(8-d); % In m^3/s.
    IT3 = @(t,d) Inflow3(8-d); % In m^3/s.
    IT4 = @(t,d) Inflow4(8-d)/2; % In m^3/s.

    f1 = @(t,V1,T,S,d) (TMax/VolMax1)*(IT1(t,d)-FlowMax1(V1)*T-MaxSpill1(V1)*S);
    f3 = @(t,V3,T,S,V,d) (TMax/VolMax3)*(IT3(t,d)-FlowMax3(V3,t)*T-MaxSpill3(V3,t)*S+MaxV3*V);
    f4 = @(t,V4,T,S,d) (TMax/VolMax4)*(IT4(t,d)-FlowMax4*T-MaxSpill4*S);

    DMax = 2.5e6; % In kW. The maximum possible value of the demand.

    Vini1 = Matrix{4}(7,1)/VolMax1*1000000;
    Vini3 = Matrix{6}(7,1)/VolMax3*1000000;
    Vini4 = VolSG(Matrix{7}(7,2));

    t21 = 1/4; % Delay between Bonete and Baygorria.
    Z21 = floor(length(time)/7*t21); % Discrete delay between Bonete and Baygorria.
    t32 = 10/24; % Delay between Baygorria and Palmar.
    Z32 = floor(length(time)/7*t32); % Discrete delay between Baygorria and Palmar.
    ...
    D = interp1(timeDemand,D,0:1/(length(time)-1):1); %+DMax*0.15*sin(4*pi*time);
    WindPower = interp1(timeDemand,WindPower,0:1/(length(time)-1):1);
    SolarPower = interp1(timeDemand,SolarPower,0:1/(length(time)-1):1);
    BioMasPower = interp1(timeDemand,BioMasPower,0:1/(length(time)-1):1);
    Export = interp1(timeDemand,Export,0:1/(length(time)-1):1);
    ...
    A = NAMin:dA:NAMax;
    if ComputePlots == 1
        set(0,'CurrentFigure',13); clf(13); set(gcf,'name','Battery Control Vs Charge');
    ...
    Khe1 = Matrix{3}(7,3); % In USD/MWh. Cost per energy of Bonete.
    Khe2 = 0; % In USD/MWh.
    Khe3 = Matrix{3}(7,2); % In USD/MWh.
    Khe4 = Matrix{3}(7,1); % In USD/MWh.
    ...
    H0 = @(uV1,uV2,uV3,uV4,t,d) IT1(t,d)*uV1/VolMax1 + IT3(t,d)*uV3/VolMax3 + IT4(t,d)*uV4/VolMax4 - Lambda21(t)*IT2(t,d);
    olCV3 = @(uV3,t) MaxTot3*(Lambda32(t) + uV3/VolMax3);

    k1 = @(t,d) IT2(t,d)/MaxV2;
    k2 = @(V3) FlowMax2(V3)/MaxV2;
    k3 = @(V3) MaxSpill2(V3)/MaxV2;
    k = @(t,V3,d) [k1(t,d),k2(V3),k3(V3)];
    ...
    d = @(V1,V3,uV1,uV2,uV3,uV4,uA,t,d) [olCT1(uV1,t,V1),olCS1(uV1,t,V1),...
        olCT2(uV2,t,V3),olCS2(uV2,t,V3),olCT3(uV3,t,V3),olCT4(uV4),olCA(uA),...
        CostTermicasNorm(d,:).*MaxTermicas(d,:)]';
    dd = @(d) diag([0,0,0,0,0,0,KA*1000000*0.000,zeros(1,length(CostTermicasNorm(d,:)))]);
    rho = KA*1000000*0.07*1;
    b = @(V1,V3,V4,t,d) [K11(V1),0,K12(V3),0,K13(V3,t),K14(V4,t),PAMax,MaxTermicas(d,:)]';
    ...
    if WhatToDo == 1 || WhatToDo == 3
        ...
        for t = length(time):-1:1
                    ...
                    for v4 = 1:length(V4)
                        for a = 1:length(A)
                            ...
                            if (v1~=1) && (v3~=1) && (v4~=1) && (a~=1) && (v1~=length(V1)) && (v3~=length(V3)) && (v4~=length(V4)) &&...
                                ...
                                if (IT1(t+1,d2)-FlowMax1(V1(v1))*ControlsFMC_Aux(v1,v3,v4,a,1)-MaxSpill1(V1(v1))*ControlsFMC_Aux(v1,v3,v4,a,2) >= 0) && Derivatives(1)
                                    uV1(v1,v3,v4,a) = (u(v1+1,v3,v4,a)-u(v1,v3,v4,a))/(dV1);
                                elseif Derivatives(1)
                                    uV1(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1-1,v3,v4,a))/(dV1);
                                end

                                if (IT3(t+1,d2)-FlowMax3(V3(v3),t+1)*ControlsFMC_Aux(v1,v3,v4,a,5)+MaxV3*ControlsFMC_Aux(v1,v3,v4,a,Pc3) >= 0) && Derivatives(2)
                                    uV3(v1,v3,v4,a) = (u(v1,v3+1,v4,a)-u(v1,v3,v4,a))/(dV3);
                                elseif Derivatives(2)
                                    uV3(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1,v3-1,v4,a))/(dV3);
                                end

                                if (IT4(t+1,d2)-FlowMax4*ControlsFMC_Aux(v1,v3,v4,a,6) >= 0) && Derivatives(3)
                            ...
                            elseif t < length(time) - 1
                                ...
                                if (v3==1 || v3==length(V3) || v4==1 || v4==length(V4) || a==1 || a==length(A)) && (v1~=1) && (v1~=length(V1)) && Derivatives(1)
                                    if IT1(t+1,d2)-FlowMax1(V1(v1))*ControlsFMC_Aux(v1,v3,v4,a,1)-MaxSpill1(V1(v1))*ControlsFMC_Aux(v1,v3,v4,a,2) >= 0
                                        uV1(v1,v3,v4,a) = (u(v1+1,v3,v4,a)-u(v1,v3,v4,a))/(dV1);
                                    else
                                        uV1(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1-1,v3,v4,a))/(dV1);
                                    end
                                ...
                                if (v1==1 || v1==length(V1) || v4==1 || v4==length(V4) || a==1 || a==length(A)) && (v3~=1) && (v3~=length(V3)) && Derivatives(2)
                                    if IT3(t+1,d2)-FlowMax3(V3(v3),t+1)*ControlsFMC_Aux(v1,v3,v4,a,5)+MaxV3*ControlsFMC_Aux(v1,v3,v4,a,Pc3) >= 0
                                        uV3(v1,v3,v4,a) = (u(v1,v3+1,v4,a)-u(v1,v3,v4,a))/(dV3);
                                    else
                                        uV3(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1,v3-1,v4,a))/(dV3);
                                    end
                                ...
                                if (v1==1 || v1==length(V1) || v3==1 || v3==length(V3) || a==1 || a==length(A)) && (v4~=1) && (v4~=length(V4)) && Derivatives(3)
                                    if IT4(t+1,d2)-FlowMax4*ControlsFMC_Aux(v1,v3,v4,a,6) >= 0
                                        uV4(v1,v3,v4,a) = (u(v1,v3,v4+1,a)-u(v1,v3,v4,a))/(dV4);
                                    else
                                        uV4(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1,v3,v4-1,a))/(dV4);
                                    end
                            ...
                            CFL(v1,v3,v4,a) = 1 / (abs(f1(t,V1(v1),ControlsFMC_Aux(v1,v3,v4,a,1),ControlsFMC_Aux(v1,v3,v4,a,2),d1))/dV1 + ...
                                abs(f3(t,V3(v3),ControlsFMC_Aux(v1,v3,v4,a,5),0,ControlsFMC_Aux(v1,v3,v4,a,Pc3),d1))/dV3 + ...
                                abs(f4(t,V4(v4),ControlsFMC_Aux(v1,v3,v4,a,6),0,d1))/dV4 + ...
                                abs(fA(ControlsFMC_Aux(v1,v3,v4,a,7)))/dA);
                            CFL_v1(v1,v3,v4,a) = abs(f1(t,V1(v1),ControlsFMC_Aux(v1,v3,v4,a,1),ControlsFMC_Aux(v1,v3,v4,a,2),d1))/dV1;
                            CFL_v3(v1,v3,v4,a) = abs(f3(t,V3(v3),ControlsFMC_Aux(v1,v3,v4,a,5),0,ControlsFMC_Aux(v1,v3,v4,a,Pc3),d1))/dV3;
                            CFL_v4(v1,v3,v4,a) = abs(f4(t,V4(v4),ControlsFMC_Aux(v1,v3,v4,a,6),0,d1))/dV4;
            ...
            parfor index = 1:TL
                ...
                Controls_Aux(index,:) = [Quad_FMC_Lambda_7(Q(V1(ind_V1),V3(ind_V3),t),b(V1(ind_V1),V3(ind_V3),...
                        V4(ind_V4),t,d1),c(t),d(V1(ind_V1),V3(ind_V3),uV1_PF(index),0,uV3_PF(index),uV4_PF(index),...
                        Aux_uA,t,d1),dd(d1),k(t,V3(ind_V3),d1),options,PhiV2Fixed,Aux_V2,WS,Aux_Lims,InfTermicas);Aux_V3];
                        ...
                        for v4 = 1:length(V4)
                            for a = 1:length(A)
                                ...
                                HAM(v1,v3,v4,a) = TMax*(Controls*[d(V1(v1),V3(v3),uV1(v1,v3,v4,a),...
                                    0,uV3(v1,v3,v4,a),uV4(v1,v3,v4,a),uA(v1,v3,v4,a),t,d1);0;...
                                    olCV3(uV3(v1,v3,v4,a),t)]+H0(uV1(v1,v3,v4,a),0,uV3(v1,v3,v4,a),uV4(v1,v3,v4,a),t,d1));
        ...
        ToSave = {HJB_Initial_Cost,UV,CFL_Plot,U};
        save([pwd '/','Simulation_',name,'.mat'],'ToSave','-v7.3');
        toc
    ...
    if WhatToDo > 1
        ...
        for t = 1:length(time)
            ...
            if t == 1
                dd_OP = @(d) dd(d);
                d_OP = @(V1,V3,uV1,uV2,uV3,uV4,uA,t,Conts,d1) d(V1,V3,uV1,uV2,uV3,uV4,uA,t,d1);
                H0_OP = @(uV1,uV2,uV3,uV4,t,Conts,d) H0(uV1,uV2,uV3,uV4,t,d);
            else
                dd_OP = @(d) dd(d) + diag(rho/dt*ones(1,12));
                d_OP = @(V1,V3,uV1,uV2,uV3,uV4,uA,t,Conts,d1) d(V1,V3,uV1,uV2,uV3,uV4,uA,t,d1) + ...
                -2*rho/dt*Conts';
                H0_OP = @(uV1,uV2,uV3,uV4,t,Conts,d) H0(uV1,uV2,uV3,uV4,t,d) + rho*sum(Conts.^2)/dt;
                Added_Cost = @(New_Conts,Conts) sum(New_Conts.^2)*rho/dt - 2*rho/dt*sum(New_Conts.*Conts) + rho/dt*sum(Conts.^2);
            end
            
            if WhatToDo ~= 4
                
                [Controls_OP(t,1:13)] = Quad_FMC_Lambda_7(Q(Vol_OP_Real_1(t),Vol_OP_Real_3(t),t),...
                    b(Vol_OP_Real_1(t),Vol_OP_Real_3(t),Vol_OP_Real_4(t),t,d1),c(t),...
                    d_OP(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,Prec_Cont(t,:),d1),dd_OP(d1),k(t,Vol_OP_Real_3(t),d1),...
                    options,1,Real2,x0,ContLims,InfTermicas);

                dd_L = @(d) dd(d);
                d_L = @(V1,V3,uV1,uV2,uV3,uV4,uA,t,Prec_Cont,d1) d(V1,V3,uV1,uV2,uV3,uV4,uA,t,d1);
                [~,lambda{t}] = Quad_FMC_Lambda_6(Q(Vol_OP_Real_1(t),Vol_OP_Real_3(t),t),...
                    b(Vol_OP_Real_1(t),Vol_OP_Real_3(t),Vol_OP_Real_4(t),t,d1),c(t),...
                    d_L(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,Prec_Cont(t,:),d1),dd_L(d1),k(t,Vol_OP_Real_3(t),d1),...
                    options,1,Real2,Controls_OP(t,1:12)',ContLims,InfTermicas);
...
%             Prev_SG(t+1) = Controls_OP(t,6);
            ...
            if UseLamb32 == 1
                if t > Z32
                    ...
                    Controls_OP(t,13) = Phi_V3(t);
                end
            else
                Controls_OP(t,13) = 0;
            end
            
            Aux_Cost = Controls_OP(t,1:12).*d(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,d1)';
            ...
            CostControls(:,t) = d(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,d1)./...
               ...
               Power_H3(1,Vol_OP_Real_3(t),t),Power_H4(1,Vol_OP_Real_4(t),t),PAMax,MaxTermicas(d1,:)]';
           ...
           % at their maximum power. For the spillages is USD/Control.
            ...
            CostControlsInstant(:,t) = d(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,d1).*Controls_OP(t,1:12)'./...
               ...
               Power_H3(Controls_OP(t,5),Vol_OP_Real_3(t),t),Power_H4(Controls_OP(t,6),Vol_OP_Real_4(t),t),PAMax,MaxTermicas(d1,:)]'+...
               ones(12,1));
           ...
           % ones(11,1) to avoid 0/0 when come control is = 0.
            ...
            if t ~= length(time)
                ...
                Vol_OP_Real_1(t+1) = Vol_OP_Real_1(t) + (dt*TMax/VolMax1)*(IT1(t,d1)-...
                    Controls_OP(t,1)*FlowMax1(Vol_OP_1(t))-Controls_OP(t,2)*MaxSpill1(Vol_OP_1(t)));
                Vol_OP_Real_3(t+1) = Vol_OP_Real_3(t) + (dt*TMax/VolMax3)*(IT3(t,d1)-...
                    Controls_OP(t,5)*FlowMax3(Vol_OP_3(t),t)+Controls_OP(t,13)*MaxV3);
                Vol_OP_Real_4(t+1) = Vol_OP_Real_4(t) + (dt*TMax/VolMax4)*(IT4(t,d1)-...
                ...
                if t == 1
                    Optimal_Cost(t+1) = Optimal_Cost(t) + dt*TMax*(Controls_OP(t,:)*...
                        [d(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,d1);0;olCV3(uV3,t)] +...
                        Controls_OP(t,1:12)*dd(d1)*Controls_OP(t,1:12)' + H0_OP(uV1,uV2,uV3,uV4,t,Prec_Cont(t,:),d1));
                else
                    
                    Optimal_Cost(t+1) = Optimal_Cost(t) + dt*TMax*(...
                    ...
                    Controls_OP(t,:)*[d_OP(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,Prec_Cont(t,:),d1);0;olCV3(uV3,t)] +...
                    ...
                    Controls_OP(t,1:12)*dd_OP(d1)*Controls_OP(t,1:12)' +...
                    ...
                    H0_OP(uV1,uV2,uV3,uV4,t,Prec_Cont(t,:),d1) -...
                    ...
                    Added_Cost(Controls_OP(t,1:12),Prec_Cont(t,:))...
                    ...
                    );
                
                    Extra_Cost = Extra_Cost + dt*TMax*Added_Cost(Controls_OP(t,12),Prec_Cont(t,:))/NormCost;
                end
            ...
            Power_F1_OP(t) = MaxTermicas(d1,1)*(Controls_OP(t,8));
            Power_F2_OP(t) = MaxTermicas(d1,2)*(Controls_OP(t,9));
            Power_F3_OP(t) = MaxTermicas(d1,3)*(Controls_OP(t,10));
            Power_F4_OP(t) = MaxTermicas(d1,4)*(Controls_OP(t,11));
            Power_F5_OP(t) = MaxTermicas(d1,5)*(Controls_OP(t,12));
        ...
        if ComputePlots == 1
            ...
            P.LineWidth = 1;
            legend('Biomass','Solar','Wind',Dams{4},Dams{1:3},Termicas{1:5},'Battery','Demand','Demand + Battery','Demand + Battery + Exp','location','eastoutside');
            xlabel('Time');
            title('Power Balance')
            ylabel('Power (MW)');
            xlim([0 max(time)]);
            ...
            set(gca,'Box','on');
                ...
                set(0,'CurrentFigure',122); clf(122); set(gcf,'name','Battery Capacity');
