function [Final_Cost] = Admissible_Solution_7(name,WhatToDo,WarmStart,UseBattery,...
...
%     CostTermicas = [Matrix{2}(1),Matrix{2}(3),Matrix{2}(5),Matrix{2}(7),600]; % USD/MWh.
    ...
    MaxSpill1 = @(V1,V2) MaxTot1 - FlowMax1(V1,V2);
    MaxSpill2 = @(V2,V3) MaxTot2 - FlowMax2(V2,V3);
    ...
    VolMax2 = 0.678e9; % We do not care about this while H2 is a constant.
    ...
    Vmax2 = min((Vini2*VolMax2+(MaxTot2)*TMax),VolMax2)/VolMax2;
    Vmin2 = max((Vini2*VolMax2-(MaxTot2)*TMax),VolMin2)/VolMax2;
    ...
    CH1 = Khe1*eta1*(dH1(Vini1,Vini2)-d1(Vini1,Vini2,1,0))/(MWhTokW); % Water's value of Bonete.
    CH2 = Khe2*eta2*(dH2(Vini2,Vini3)-d2(Vini2,Vini3,1,0))/(MWhTokW); % Water's value of Baygorria.
    ...
    olCA = @(uA) -K_A*uA;
    olCV2 = @(uV2,t) MaxTot2*(Lambda21(t) + uV2/VolMax2);
    olCV3 = @(uV3,t) MaxTot3*(Lambda32(t) + uV3/VolMax3);
    H0 = @(uV1,uV2,uV3,uV4,t) IT1(t)*uV1/VolMax1 + IT2(t)*uV2/VolMax2 + IT3(t)*uV3/VolMax3 + IT4(t)*uV4/VolMax4;
    
    H0_1 = @(uV1,t) IT1(t)*uV1/VolMax1;
    H0_2 = @(uV2,t) IT2(t)*uV2/VolMax2;
    ...
    d = @(V1,V2,V3,uV1,uV2,uV3,uV4,uA,t) [olCT1(uV1,t,V1,V2),olCS1(uV1,t,V1,V2),...
        olCT2(uV2,t,V2,V3),olCS2(uV2,t,V2,V3),olCT3(uV3,t,V3),olCT4(uV4),olCA(uA),...
        CostTermicasNorm.*MaxTermicas]';
    dd = diag([0,0,0,0,0,0,KA*1000000*0.000,zeros(1,length(CostTermicasNorm))]);
    rho = KA*1000000*0.07*0.5;
    if WhatToDo == 6
        rho = 0;
    end
    ...
    if WhatToDo == 1 || WhatToDo == 3
        ...
        Final_Cost = u(IP1,IP2,IP3,IP4,IPA)/NormCost;
        ...
        if WhatToDo ~= 5
            ...
            for t = 1:length(time)
                ...
                elseif WhatToDo == 4
                    ...
                    Controls_OP(t,1:12) = Controls_OP(t,1:12)*1000./[Power_H1(1,0,Vol_OP_Real_1(t),Vol_OP_Real_2(t)),1,...
                    Power_H2(1,0,Vol_OP_Real_2(t),Vol_OP_Real_3(t)),1,...
                ...
                if UseLamb21 == 1
                    if t > Z21
                        Controls_OP(t,13) = Phi_V2_Aux(t-Z21);
                        if Fixed == 0
                            if olCV2(uV2,t) > 0
                                Controls_OP(t,13) = 0;
                            else
                ...
                WaterBonete(t) =  dt*TMax*(Controls_OP(t,1)*FlowMax1(Vol_OP_Real_1(t),Vol_OP_Real_2(t)) + Controls_OP(t,2)*MaxSpill1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))); 
                WaterVirtualBay(t) = dt*TMax*Controls_OP(t,13)*MaxV2;

                Aux_Cost = dt*TMax*Controls_OP(t,1:12).*d(Vol_OP_Real_1(t),Vol_OP_Real_2(t),Vol_OP_Real_3(t),uV1,uV2,uV3,uV4,uA,t)';
               ...
               % at their maximum power. For the spillages is USD/Control.
                ...
                CostControlsInstant(:,t) = d(Vol_OP_Real_1(t),Vol_OP_Real_2(t),Vol_OP_Real_3(t),uV1,uV2,uV3,uV4,uA,t).*Controls_OP(t,1:12)'./...
                   ([Power_H1(Controls_OP(t,1),Controls_OP(t,2),Vol_OP_Real_1(t),Vol_OP_Real_2(t)),MaxSpill1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))*dt*TMax/m3Tohm3,...
                   Power_H2(Controls_OP(t,3),Controls_OP(t,4),Vol_OP_Real_2(t),Vol_OP_Real_3(t)),MaxSpill2(Vol_OP_Real_2(t),Vol_OP_Real_3(t))*dt*TMax/m3Tohm3,...
               ...
               % ones(11,1) to avoid 0/0 when come control is = 0.
                ...
                if t ~= length(time)
                    ...
                    Optimal_Cost(t+1) = Optimal_Cost(t) + dt*TMax*(Controls_OP(t,:)*...
                        [d_OP(Vol_OP_Real_1(t),Vol_OP_Real_2(t),Vol_OP_Real_3(t),uV1,uV2,uV3,uV4,uA,t,Prec_Cont(t,:));olCV2(uV2,t);olCV3(uV3,t)] +...
            ...
            if ComputePlots == 1
                ...
                plot(time,Vol_OP_2); hold on;
                plot(time,Vol_OP_Real_2);
                ylim([Vmin2 Vmax2]);
                grid on; grid minor;
                xlabel('Time');
                ylabel('Volume');
                title('Volume Baygorria');
                legend('Approximated Volume Optimal Path','Real Volume Optimal Path');
                xlim([0 max(time)]);
                set(gca,'Box','on');
                ...
                if WhatToDo == 4
                    sets = [102,103,104,110,112,116,117,121,122,123,124,125,126,127];
                    for m = 1:length(sets)
                        i = sets(m);
                        set(0,'CurrentFigure',i);
