function [Uini] = Cost_Ini(WeUseLambda,WithWind,SaveFig,SaveU,ShowFig,ComputePlots,Use_Fmincon,expansion,Fcost,Lag,Hat_Lag)
...
%0.8 3/10
D = DMax*(0.5+(0.5)*sin(3*pi*time)); % In kW.
...
HamToMin = @(uv4,uv1,uvv4,uvv1,uw,uww,phit4,phit1,phif,Vphit4,t,Y_hat) (uv4*TMax/VolMax4*FlowMax1+Lag(t))*Vphit4*WeUseLambda + TMax*(hatKh4(uv4)*phit4 + (hatKh1(uv1)-Hat_Lag(t+Z41)*WeUseLambda)*phit1 + hatKf*phif + IT4(t)*uv4/VolMax4 + IT1(t)*uv1/VolMax1) + (sigH1^2)/2*uvv1 + (sigH4^2)/2*uvv4 + (d_pW(t)+theta_W(RealTime(t))*(pW(t)-Y_hat))*uw + theta_W(RealTime(t))*alpha_W*Y_hat*(1-Y_hat)*uww;
HamToMin_HF = @(uv4,uv1,uvv4,uvv1,phit4,phit1,phif,Vphit4,t,Y_hat) (uv4*TMax/VolMax4*FlowMax1+Lag(t))*Vphit4*WeUseLambda + TMax*(hatKh4(uv4)*phit4 + (hatKh1(uv1)-Hat_Lag(t+Z41)*WeUseLambda)*phit1 + hatKf*phif + IT4(t)*uv4/VolMax4 + IT1(t)*uv1/VolMax1) + (sigH1^2)/2*uvv1 + (sigH4^2)/2*uvv4;
    ...
    for k=WindLow:WindHigh
        ...
        if t~=length(time)
            if t~=1
                ...
                u(t,:,:,k) = u(t+1,:,:,k) + dt*reshape(HAM,[1,length(V1),length(V4)]) - Fcost*WeUseLambda;
            end
        end
