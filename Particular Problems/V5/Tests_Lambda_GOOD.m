...
WithOut_Lambda = Cost_Ini_V4Constant(0,0,0,0,0,0,0,expansion,Lambda(A(1)),[Lambda(A(1)),zeros(1,Z41)]);

parfor i=1:length(A)
%    U(i) = Cost_Ini(1,0,0,0,0,0,0,expansion,Lambda(A(i)),[Lambda(A(i)),zeros(1,Z41)]);
    U(i) = Cost_Ini_V4Constant(1,0,0,0,0,0,0,expansion,Lambda(A(i)),[Lambda(A(i)),zeros(1,Z41)]);
    Y(i) = WithOut_Lambda;
end
...
% grid minor;
% title('Cost Function');
% xlabel('Constant Value of $\lambda(t)$','interpreter','latex');
% ylabel('Cost');
% legend('Linked','No Linked')
% [maxU,idxU] = max(U(:));
...
U0 = {A,U,Y};
save('Exp_0','U0');
saveas(gcf,'Test_Lambda_2','epsc');

% up we have all what we need here, we have to change the commend lines
