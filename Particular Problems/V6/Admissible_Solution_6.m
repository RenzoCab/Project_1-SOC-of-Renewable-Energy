function [HJB_Initial_Cost] = Admissible_Solution_6(name,WhatToDo,WarmStart,UseBattery,...
    ...
    Termicas = {'CTR','Motores Batlle','PTA','PTB'};
    Dams = {'Bonete','Baygorria','Palmar','Salto Grande'};
    MaxTermicas = [Matrix{2}(2),Matrix{2}(4),Matrix{2}(6),Matrix{2}(8)]*1000; % kW.
    InfTermicas = [0,0,0,0];
    CostTermicas = [Matrix{2}(1),Matrix{2}(3),Matrix{2}(5),Matrix{2}(7)]; % USD/MWh.
        ...
        for t = length(time):-1:1
                    ...
                    for v4 = 1:length(V4)
                        for a = 1:length(A)
                            ...
                            if (v1~=1) && (v3~=1) && (v4~=1) && (a~=1) && (v1~=length(V1)) && (v3~=length(V3)) && (v4~=length(V4)) &&...
                                ...
                                if (IT3(t+1)-FlowMax3(V3(v3),t+1)*ControlsFMC_Aux(v1,v3,v4,a,5)+MaxV3*ControlsFMC_Aux(v1,v3,v4,a,13) >= 0) && Derivatives(2)
                            ...
                            elseif t < length(time) - 1
                                ...
                                if (v1==1 || v1==length(V1) || v4==1 || v4==length(V4) || a==1 || a==length(A)) && (v3~=1) && (v3~=length(V3)) && Derivatives(2)
                                    if IT3(t+1)-FlowMax3(V3(v3),t+1)*ControlsFMC_Aux(v1,v3,v4,a,5)+MaxV3*ControlsFMC_Aux(v1,v3,v4,a,13) >= 0
                                        uV3(v1,v3,v4,a) = (u(v1,v3+1,v4,a)-u(v1,v3,v4,a))/(dV3);
                                    else
                                        uV3(v1,v3,v4,a) = (u(v1,v3,v4,a)-u(v1,v3-1,v4,a))/(dV3);
                                    end
            ...
            parfor index = 1:TL
                ...
                Controls_Aux(index,:) = [Quad_FMC_Lambda_7(Q(V1(ind_V1),V3(ind_V3),t),b(V1(ind_V1),V3(ind_V3),...
                        V4(ind_V4),t),c(t),d(V1(ind_V1),V3(ind_V3),uV1_PF(index),0,uV3_PF(index),uV4_PF(index),...
    ...
    if WhatToDo == 2 || WhatToDo == 3
        ...
        CostControlsInstant = zeros(11,length(time));
        ...
        for t = 1:length(time)
            ...
            [Controls_OP(t,1:12)] = Quad_FMC_Lambda_7(Q(Vol_OP_Real_1(t),Vol_OP_Real_3(t),t),...
            ...
            [~,lambda{t}] = Quad_FMC_Lambda_6(Q(Vol_OP_Real_1(t),Vol_OP_Real_3(t),t),...
            	b(Vol_OP_Real_1(t),Vol_OP_Real_3(t),Vol_OP_Real_4(t),t),c(t),...
            	d_L(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t,Prev_SG(t),Prev_Batt(t),Prev_F4(t)),dd_L,k(t,Vol_OP_Real_3(t)),...
                options,1,Real2,Controls_OP(t,1:11)',ContLims,InfTermicas);
           ...
           % at their maximum power. For the spillages is USD/Control.
            ...
            CostControlsInstant(:,t) = d(Vol_OP_Real_1(t),Vol_OP_Real_3(t),uV1,0,uV3,uV4,uA,t).*Controls_OP(t,1:11)'./...
        ...
        if ComputePlots == 1
            ...
            hh(12).FaceColor = [1 1 0]; % Battery.
            P = plot(time,D,'k'); P.LineWidth = 1;
            P = plot(time,D-squeeze(Controls_OP(:,7))'*PAMax.*(squeeze(Controls_OP(:,7))' < 0),'c');
            P.LineWidth = 1;
            P = plot(time,D+Export-squeeze(Controls_OP(:,7))'*PAMax.*(squeeze(Controls_OP(:,7))' < 0),'m');
            P.LineWidth = 1;
            legend('Biomass','Solar','Wind',Dams{4},Dams{1:3},Termicas{1:4},'Battery','Demand','Demand + Battery','Demand + Battery + Exp.','location','eastoutside');
            xlabel('Time');
            title('Power Balance')
            ylabel('Power (kW)');
            xlim([0 max(time)]);
            ...
            grid on; grid minor;
            title('Costs in the Optimal Path');
            legend(Termicas{:},'Battery'); xlabel('Time'); ylabel('USD/MWh');
            ...
            X = [sum(BioMasPower),sum(SolarPower),sum(WindPower),sum(Power_H1_OP),sum(Power_H2_OP),...
                ...
                sum(Power_F4_OP),sum(Power_Battery_OP.*(Power_Battery_OP > 0)),];
            txt = {'Biomass: ','Solar Power: ','Wind Power: ',[Dams{1},': '],[Dams{2},': '],...
                [Dams{3},': '],[Dams{4},': '],[Termicas{1},': '],[Termicas{2},': '],...
                [Termicas{3},': '],[Termicas{4},': '],'Battery: '};
            ...
            hh(9).FaceColor = [1 1 0]; % Battery.
            legend(Dams{4},Dams{1:3},Termicas{1:4},'Battery','location','eastoutside');
            xlabel('Time');
