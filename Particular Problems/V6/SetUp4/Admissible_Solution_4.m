function [HJB_Initial_Cost] = Admissible_Solution_4(OptimalPath,ComputePlots,SavePlots,Expan,Expan_Time,UseLamb21,UseLamb32,Lambda21,Lambda32)
    ...
    Vini3 = 0.65; % Initial condition between 0.6 and 1.
...
%     D = DMax*(0.5)*time./time; % In kW. (THIS (2))
    ...
    for t = length(time):-1:1
            ...
            for v3 = 1:length(V3)
                ...
                for v4 = 1:length(V4)
                    ...
                    if (v1~=1) && (v3~=1) && (v4~=1) && (v1~=length(V1)) && (v3~=length(V3)) && (v4~=length(V4)) && (t<length(time)-1)
                        ...
                        if (IT3(t+1)-FlowMax3*ControlsFMC_Aux(v1,v3,v4,6)+MaxV3*ControlsFMC_Aux(v1,v3,v4,8) >= 0) && Derivatives(2)
        ...
        if ComputePlots == 1
            ...
            hold on;
            for index=1:1:length(V4)
                surf(vplot1_4,vplot3_4,squeeze(HAM(:,:,index))');
            end
            xlabel('Vol. Bonete');
            ylabel('Vol. Palmar');
            ...
            hold on;
            for index=1:1:length(V3)
                surf(vplot1_3,vplot4_3,squeeze(HAM(:,index,:))');
            end
            xlabel('Vol. Bonete');
            ylabel('Vol. Salto Grande');
            title(['Hamiltonian at time = ',num2str(t)]);
            ...
            hold on;
            for index=1:1:length(V1)
                surf(vplot3_1,vplot4_1,squeeze(HAM(index,:,:))');
            end
            xlabel('Vol. Palmar');
            ylabel('Vol. Salto Grande');
            title(['Hamiltonian at time = ',num2str(t)]);
            ...
            hold on;
            for index=1:1:length(V4)
                surf(vplot1_4,vplot3_4,squeeze(uV4(:,:,index))');
            end
            xlabel('Vol. Bonete');
            ylabel('Vol. Palmar');
            ...
            hold on;
            for index=1:1:length(V3)
                surf(vplot1_3,vplot4_3,squeeze(uV3(:,index,:))');
            end
            xlabel('Vol. Bonete');
            ylabel('Vol. Salto Grande');
            ...
            hold on;
            for index=1:1:length(V1)
                surf(vplot3_1,vplot4_1,squeeze(uV1(index,:,:))');
            end
            xlabel('Vol. Palmar');
            ylabel('Vol. Salto Grande');
