...
realIniTime = 0; % 00:00 hours.
realFinTime = 24; % 24:00 hours.
RealTime = realIniTime:(realFinTime-realIniTime)/(length(time)-1):realFinTime; % Time vector in hours.
...
% Next we load all the data about the Wind. In particular, a 4x25 matrix
% named Wind_Data which has: (1,:) expect value or forecast, (2,:) the
% derivative of the previous row, (3,:) the sigma of the SDE and in (4,:)
% the derivative of the previous row.
load('Wind_Data.mat');

Gk = 1383*10^3; % kW. Maximum wind power.
...
MinW = 0.4; % Maximum wind (it is between 0 and 1).
MaxW = 0.6; % Minimum wind (it is between 0 and 1).
dw = (MaxW-MinW)/NW;
...
Khe1 = 0.3; % In USD/MWh. Cost per energy of Bonete.
Khe2 = 0; % In USD/MWh.
Khe3 = 0.3; % In USD/MWh.
Khe4 = 0.3; % In USD/MWh.
Kf = 130; % In USD/MWh. Cost per energy of the fuel.
...
for t = length(time):-1:1
        ...
        for v3 = 1:length(V3)
            ...
            for v4 = 1:length(V4)
                ...
                if (v1~=1) && (v3~=1) && (v4~=1) && (v1~=length(V1)) && (v3~=length(V3)) && (v4~=length(V4)) && (t<length(time)-1)
                    ...
                    if IT1(t+1)-FlowMax1*ControlsFMC(v1,v3,v4,2)-MaxSpill1*ControlsFMC(v1,v3,v4,3) >= 0
                        uV1(v1,v3,v4) = (u(v1+1,v3,v4)-u(v1,v3,v4))/(dV1);
                    else
                        uV1(v1,v3,v4) = (u(v1,v3,v4)-u(v1-1,v3,v4))/(dV1);
                    end
                    
                    if IT3(t+1)-FlowMax3*ControlsFMC(v1,v3,v4,6)+MaxV3*ControlsFMC(v1,v3,v4,8) >= 0
                        uV3(v1,v3,v4) = (u(v1,v3+1,v4)-u(v1,v3,v4))/(dV3);
                    else
                        uV3(v1,v3,v4) = (u(v1,v3,v4)-u(v1,v3-1,v4))/(dV3);
                    end
                    
                    if IT4(t+1)-FlowMax4*ControlsFMC(v1,v3,v4,7) >= 0
    ...
    for v1 = 1:length(V1)
        ...
        for v3 = 1:length(V3)
            ...
            for v4 = 1:length(V4)
                ...
                ControlsFMC(v1,v3,v4,1:8) = Quad_FMC_P(Q,b(V1(v1),V3(v3),V4(v4)),c(t),...
    ...
    for v1 = 1:length(V1)
        ...
        for v3 = 1:length(V3)
            ...
            for v4 = 1:length(V4)
                ...
                Controls(1:9) = ControlsFMC(v1,v3,v4,:);
    ...
    figure;
    [VV1,VV3] = meshgrid(V1,V3);
    hold on;
    
    for index=1:length(V4)
            s1 = surf(VV1,VV3,squeeze(u(:,:,index))');
            s1.EdgeColor = 'none';
    end
