function [Minimum] = Min(Q,b,c,d,k)
    ...
    while Done == 0
        ...
        if m == 0
            ...
            T_Lambda_F = b(1)/d(1);
            [nQ,nb,nc,nd] = Mat_Red(Q,b,c,d,1,0);
            Y = T_Solver(nQ,nb,nd,T_Lambda_F); % Y = [x2,x3,x4,x5,x6,x7].
                ...
                for j = 1:length(bins(:,1))
                    ...
                    if (ismember(4,sets(i,:))&&(ismember(5,sets(i,:)))) || (ismember(4,sets(i,:))&&(ismember(8,sets(i,:)))) || (ismember(5,sets(i,:))&&(ismember(8,sets(i,:))))
                        ...
                        if isreal([x4,x5,x8])
                            ...
                            if ismember(5,sets_aux)
                                bins_aux(sets_aux==5) = [];
                                sets_aux(sets_aux==5) = [];
                            end
                            if ismember(4,sets_aux)
                                bins_aux(sets_aux==4) = [];
                                sets_aux(sets_aux==4) = [];
                            ...
                            if ~isempty(sets_aux) && sets_aux(1) == 1 && bins_aux(1) == 0 % This means Phi_F=0 which implies NT solutions.
                                ...
                                if ismember(1,sets_aux) && not(ismember(2,sets_aux)) % This means T1 fixed and S1 free.
                                    ...
                                    if length(nQ) ~= length(sets_aux)
                                        ...
                                        if isreal(Y)
                                            Y = [x1,x2,x4,x5,Y];
                                            [sQ,sb,sc,sd] = Mat_Red(Q,b,c,d,3,0);
                            ...
                            elseif ~isempty(sets_aux) && sets_aux(1) == 1 && bins_aux(1) == 1
                                ...
                                if ismember(2,sets_aux) && not(ismember(3,sets_aux)) % T1 fixed and S1 free.
                                    T_Lambda_F = nb(1)/nd(1);
                                    % Puedo ignorar c y nc.
                                    nQ = nQ(2:end,2:end);
                                    nb = nb(2:end);
                                    nd = nd(2:end);
                                    [nQ,nb,nc,nd] = Mat_Red(nQ,nb,nc,nd,1,bins_aux(1)); % Reduzco T1.
                    ...
                    elseif sets(i,end) == 8 % 4 and 5 are not in sets(i,:).
                        ...
                        if sets(i,1) == 1 && bins(j,1) == 0 % This means Phi_F = 0.
                            ...
                            if ismember(1,sets_aux) && not(ismember(2,sets_aux)) % T1 fixed and S1 free.
                                
                                if ismember(3,sets_aux) && not(ismember(4,sets_aux)) % T1-F, S1-NF, T2-F and S2-NF.
                        ...
                        elseif sets(i,1) == 1 && bins(j,1) == 1
                            ...
                            if  ismember(2,sets_aux) && not(ismember(3,sets_aux))
                                ...
                                if  ismember(4,sets_aux) && not(ismember(5,sets_aux))
                                    ...
                                    [nQ,nb,nc,nd] = Mat_Red_Bins(Q,b,c,d,sets_aux,bins_aux);
                            ...
                            elseif ismember(4,sets_aux) && not(ismember(5,sets_aux)) % && not(ismember(2,sets_aux) && not(ismember(3,sets_aux)))
                                ...
                                T_Lambda_F = nb(1)/nd(1);
                                [nQ,nb,nc,nd] = Mat_Red_Bins(nQ,nb,nc,nd,1,0); % This removes Phi_F and reduces the system.
                                sets_aux = sets_aux - 1;
