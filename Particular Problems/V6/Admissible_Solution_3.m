function [HJB_Initial_Cost] = Admissible_Solution_3(OptimalPath,ComputePlots,SavePlots,Expan,Expan_Time,UseLamb21,UseLamb32,Lambda21,Lambda32)
...
%     Vmin4 = max((Vini4*VolMax4+(IT4(1)-FlowMax4)*TMax),VolMin4)/VolMax4;
    ...
    x0 = zeros(length(V1),length(V3),length(V4),7);
    ...
    for t = length(time):-1:1
        ...
        parfor v1 = 1:length(V1)
            
            Controls_Aux_2 = zeros(length(V3),length(V4),9);
            
            for v3 = 1:length(V3)
                
                Aux_V3 = zeros(1,length(V4));
                Controls_Aux = zeros(length(V4),9);
                ...
                for v4 = 1:length(V4)
                    ...
                    WarmStart = squeeze(x0(v1,v3,v4,:));
                    Controls_Aux(v4,:) = [Quad_FMC_Lambda(Q,b(V1(v1),V3(v3),V4(v4)),c(t),...
                            d(uV1(v1,v3,v4),0,uV3(v1,v3,v4),uV4(v1,v3,v4),t),...
                            k(t),options,PhiV2Fixed,Aux_V2,WarmStart);Aux_V3(v4)];

                end
                
                Controls_Aux_2(v3,:,:) = Controls_Aux;
                
            end
            
            ControlsFMC(v1,:,:,:) = Controls_Aux_2;
        ...
        if ComputePlots == 1
            ...
            for index=1:1:length(V4)
                surf(vplot1_4,vplot3_4,squeeze(u(:,:,index))');
            end
            xlabel('Vol. Bonete');
            ylabel('Vol. Palmar');
            zlabel('USD');
            ...
            for index=1:1:length(V3)
                surf(vplot1_3,vplot4_3,squeeze(u(:,index,:))');
            end
            xlabel('Vol. Bonete');
            ...
            for index=1:1:length(V1)
                surf(vplot3_1,vplot4_1,squeeze(u(index,:,:))');
            end
            xlabel('Vol. Palmar');
            ylabel('Vol. Salto Grande');
            zlabel('USD');
