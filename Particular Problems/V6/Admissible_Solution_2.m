function [HJB_Initial_Cost] = Admissible_Solution_2(OptimalPath,ComputePlots,SavePlots,expansion,UseLamb21,UseLamb32,Lambda21,Lambda32)

    tic
    
    x0 = zeros(7,1); % The initial point of the optimisation.
    ...
    options = optimoptions('fmincon','Display','notify','MaxFunctionEvaluations',10000,...
        'FunctionTolerance',1.0000e-9,'MaxIterations',5000,'MaxFunEvals',10^7,'Algorithm','sqp','ConstraintTolerance',0.1);
    ...
    NV1 = 2^expansion; % Discretizations of Bonete.
    NV2 = 2^expansion; % Discretizations of Baygorria.
    NV3 = 2^expansion; % Discretizations of Palmar.
    NV4 = 2^expansion; % Discretizations of Salto Grande.
...
%     Vmin1 = max((Vini1*VolMax1+(IT1(1)-FlowMax1-MaxSpill1)*TMax),VolMin1)/VolMax1;
%     Vmax2 = min((Vini2*VolMax2+IT2(1)*TMax),VolMax2)/VolMax2;
%     Vmin2 = max((Vini2*VolMax2+(IT2(1)-FlowMax2-MaxSpill2)*TMax),VolMin2)/VolMax2;
%     Vmax3 = min((Vini3*VolMax3+IT3(1)*TMax),VolMax3)/VolMax3;
% %     Vmin3 = max((Vini3*VolMax3+(IT3(1)-FlowMax3-MaxSpill3)*TMax),VolMin3)/VolMax3;
%     Vmin3 = max((Vini3*VolMax3+(IT3(1)-FlowMax3)*TMax),VolMin3)/VolMax3;
%     Vmax4 = min((Vini4*VolMax4+IT4(1)*TMax),VolMax4)/VolMax4;
% %     Vmin4 = max((Vini4*VolMax4+(IT4(1)-FlowMax4-MaxSpill4)*TMax),VolMin4)/VolMax4;
%     Vmin4 = max((Vini4*VolMax4+(IT4(1)-FlowMax4)*TMax),VolMin4)/VolMax4;
    ...
    D = DMax*(0.6-(0.2)*sin(3*pi*time)); % In kW. (THIS (1))

    % D = DMax*(0.5)*time./time; % In kW. (THIS (2))

%     D = D01012017'*1000; % In kW. (THIS (3))
%     D = interp1(absTime,D,time); % Interpolation. (THIS (3))
    ...
    Kf = 130; % In Mil USD/MWh. Cost per energy of the fuel.
        ...
        for v1 = 1:length(V1)
            ...
            for v3 = 1:length(V3)
                ...
                for v4 = 1:length(V4)
                    ...
                    if UseLamb21 == 1
                        if t > Z21
                            ControlsFMC(v1,v3,v4,1:8) = Quad_FMC_Lambda(Q,b(V1(v1),V3(v3),V4(v4)),c(t),...
                                d(uV1(v1,v3,v4),0,uV3(v1,v3,v4),uV4(v1,v3,v4),t),k(t),options,0,0,x0);
                        else
                            ControlsFMC(v1,v3,v4,1:8) = Quad_FMC_Lambda(Q,b(V1(v1),V3(v3),V4(v4)),c(t),...
                                d(uV1(v1,v3,v4),0,uV3(v1,v3,v4),uV4(v1,v3,v4),t),k(t),options,1,Phi_V2,x0);
                        end
                    else % if UseLamb21 == 0
                        ControlsFMC(v1,v3,v4,1:8) = Quad_FMC_Lambda(Q,b(V1(v1),V3(v3),V4(v4)),c(t),...
                            d(uV1(v1,v3,v4),0,uV3(v1,v3,v4),uV4(v1,v3,v4),t),k(t),options,1,0,x0);
                    end
                    
                    x0(1:7) = ControlsFMC(v1,v3,v4,1:7);
                    
                    if (UseLamb32 == 0) || (olCV3(uV3(v1,v3,v4),t) >= 0 && t > Z32)
                        ControlsFMC(v1,v3,v4,9) = 0;
                    elseif t > Z32
                        ControlsFMC(v1,v3,v4,9) = 1;
                    else
                        ControlsFMC(v1,v3,v4,9) = Phi_V3;
                    end
...
%                 S.EdgeColor = 'none';
    ...
    IP1 = floor(length(V1)/2) + 1; % Central and initial point.
    IP3 = IP1; IP4 = IP1;
    ...
    if OptimalPath == 1
        ...
        for t = 1:length(time)
            ...
            Aux = squeeze(Controls_OP(t,1:7))';
            Controls_OP(t,1:7)*Q*Aux+Controls_OP(t,1:7)*b(Vol_OP_1(t),Vol_OP_3(t),Vol_OP_4(t))+c(t)
            ...
            if t ~= length(time)
                ...
                Vol_OP_1(t+1) = Vol_OP_1(t) + (dt*TMax/VolMax1)*(IT1(t)-Controls_OP(t,2)*FlowMax1-Controls_OP(t,3)*MaxSpill1);
                Vol_OP_3(t+1) = Vol_OP_3(t) + (dt*TMax/VolMax3)*(IT3(t)-Controls_OP(t,6)*FlowMax3-Controls_OP(t,6)*MaxSpill3+Controls_OP(t,9)*MaxV3);
                Vol_OP_4(t+1) = Vol_OP_4(t) + (dt*TMax/VolMax4)*(IT4(t)-Controls_OP(t,7)*FlowMax4-Controls_OP(t,7)*MaxSpill4);

                % This is to approximate with the point of the grid.
                Vol_OP_1(t+1) = V1(min(abs(V1-Vol_OP_1(t+1))) == abs(V1-Vol_OP_1(t+1)));
                Vol_OP_3(t+1) = V3(min(abs(V3-Vol_OP_3(t+1))) == abs(V3-Vol_OP_3(t+1)));
                Vol_OP_4(t+1) = V4(min(abs(V4-Vol_OP_4(t+1))) == abs(V4-Vol_OP_4(t+1)));

                IP1 = find(V1 == Vol_OP_1(t+1));
                IP3 = find(V3 == Vol_OP_3(t+1));
                IP4 = find(V4 == Vol_OP_4(t+1));

                Optimal_Cost(t+1) = Optimal_Cost(t) + dt*TMax*(Controls_OP(t,:)*[d(uV1,0,uV3,uV4,t);0;olCV3(uV3,t)] + H0(uV1,uV2,uV3,uV4,t));
        ...
        if ComputePlots == 1
            ...
            hold on;
            plot(time,Optimal_Cost);
            plot(time,ones(1,length(time))*HJB_Initial_Cost);
            grid on;
            legend('Accumulated Cost','HJB Final Cost');
            xlabel('Normalized Time');
            ylabel('USD');
