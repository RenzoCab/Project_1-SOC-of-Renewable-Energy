function [HJB_Initial_Cost] = Admissible_Solution_4(WarmStart,OptimalPath,ComputePlots,NewPlots,SavePlots,Expan,Expan_Time,UseLamb21,UseLamb32,Lambda21,Lambda32)
...
%             for index=1:1:length(V4)
                S = surf(vplot1_4,vplot3_4,squeeze(HAM(:,:,index))'/1e5);
                S.EdgeColor = 'none';
%             end
            xlabel('Vol. Bonete');
            ylabel('Vol. Palmar');
            zlabel('Normalized Hamiltonian');
...
%             for index=1:1:length(V3)
                S = surf(vplot1_3,vplot4_3,squeeze(HAM(:,index,:))'/1e5);
                S.EdgeColor = 'none';
%             end
            xlabel('Vol. Bonete');
            ylabel('Vol. Salto Grande');
...
%             for index=1:1:length(V1)
                S = surf(vplot3_1,vplot4_1,squeeze(HAM(index,:,:))'/1e5);
                S.EdgeColor = 'none';
%             end
            xlabel('Vol. Palmar');
            ylabel('Vol. Salto Grande');
...
%             end
    ...
    if OptimalPath == 1
        ...
        for t = 1:red_dt:length(time) + rest
            ...
            uV1plot(t) = uV1; uV3plot(t) = uV3; uV4plot(t) = uV4;
        ...
        if ComputePlots == 1
            ...
            clf(111);
            grid on; hold on;
            hh = area(time,[Power_H2_OP;Power_H1_OP;Power_H3_OP;Power_H4_OP;Power_F_OP]');
            [~,h_legend] = legend('Baygorria','Bonete','Palmar','Salto Grande','Fuel');
            hh(1).FaceColor = [.5 .5 1];
            hh(2).FaceColor = [.5 .8 1];
            hh(3).FaceColor = [.3 .3 1];
            hh(4).FaceColor = [0 0 1];
            hh(5).FaceColor = [1 .5 .5];
            plot(time,D,'*');
            xlabel('Normalized Time');
            title('Power Balance')
            ylabel('Power (kW)');
            ...
            set(0,'CurrentFigure',120);
            clf(120);
            hold on;
            plot(time,uV1plot/1e5); plot(time,uV3plot/1e5); plot(time,uV4plot/1e5);
            grid on;
            Leg2 = legend('$u_{V1}$','$u_{V3}$','$u_{V4}$','location','southeast');
