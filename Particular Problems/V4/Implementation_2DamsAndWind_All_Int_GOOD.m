...
% To have the wind forecast plot, evaluate this:
% load('Wind_Data.mat');
% plot([0:24],Wind_Data(1,:));
% xlim([0 24]);
% grid minor;
% title('Wind Power Forecast');
% ylabel('Wind Power');
% xlabel('Time');
% saveas(gcf,'Wind_Forecast','epsc');

% To have the Demand plot, evaluate this:
% load('D01012017.mat');
% plot([0:24],D01012017/2000);
% xlim([0 24]);
% grid minor;
% title('Demand');
% ylabel('Demand');
% xlabel('Time');
% saveas(gcf,'Demand_Wind','epsc');
...
HamToMin = @(uv4,uv1,uvv4,uvv1,phit4,phit1,phif,t,cw,Y_hat) TMax*(hatKh4(uv4)*phit4 + hatKh1(uv1)*phit1 + hatKf*phif + IT4(t)*uv4/VolMax4 + IT1(t)*uv1/VolMax1) + (sigH1^2)/2*uvv1 + (sigH4^2)/2*uvv4 + (-Y_hat*(theta_W(RealTime(t))+d_SigW(t)/SigW(t)))*(6*cw(1)*Y_hat^5+5*cw(2)*Y_hat^4+4*cw(3)*Y_hat^3+3*cw(4)*Y_hat^2+2*cw(5)*Y_hat+cw(6)) + (4*theta_W(RealTime(t))*alpha_W/(SigW(t)^2))*(Y_hat*SigW(t)/2+pW(t))*(1-Y_hat*SigW(t)/2-pW(t))*(6*5*cw(1)*Y_hat^4+5*4*cw(2)*Y_hat^3+4*3*cw(3)*Y_hat^2+3*2*cw(4)*Y_hat+2*cw(5));
HamToMin_HF = @(uv4,uv1,uvv4,uvv1,phit4,phit1,phif,t,cw,Y_hat) TMax*(hatKh4(uv4)*phit4 + hatKh1(uv1)*phit1 + hatKf*phif + IT4(t)*uv4/VolMax4 + IT1(t)*uv1/VolMax1) + (sigH1^2)/2*uvv1 + (sigH4^2)/2*uvv4;
HamToMin_W = @(uv4,uv1,phit4,phit1,phif,t,cw,Y_hat) (-Y_hat*(theta_W(RealTime(t))+d_SigW(t)/SigW(t)))*(6*cw(1)*Y_hat^5+5*cw(2)*Y_hat^4+4*cw(3)*Y_hat^3+3*cw(4)*Y_hat^2+2*cw(5)*Y_hat+cw(6)) + (4*theta_W(RealTime(t))*alpha_W/(SigW(t)^2))*(Y_hat*SigW(t)/2+pW(t))*(1-Y_hat*SigW(t)/2-pW(t))*(6*5*cw(1)*Y_hat^4+5*4*cw(2)*Y_hat^3+4*3*cw(3)*Y_hat^2+3*2*cw(4)*Y_hat+2*cw(5));
...
for t=length(time):-1:1
    ...
    % the idea is to see the effect of the polinomial interpolation in the
    % wind, after the breakpoint jumps (t==180 just after this comments), we just press
%     set(0,'CurrentFigure',17);
%     grid minor
%   saveas(gcf,'All_Int_1','epsc');
%     set(0,'CurrentFigure',6);
%     grid minor;
%   saveas(gcf,'All_Int_2','epsc');
    % to make the plots 6 and 17 nice, and we save them.
        ...
        for i=1+Reduction:length(V1)-Reduction
            for j=1+Reduction:length(V4)-Reduction
                ...
                if (i~=1+Reduction) && (j~=1+Reduction) && (i~=length(V1)-Reduction) && (j~=length(V4)-Reduction) && (t~=length(time))
                    ...
                    uvv1(i,j,w) = (u(t+1,i+1,j,w)-2*u(t+1,i,j,w)+u(t+1,i-1,j,w))/(dV1^2);
                    uv4(i,j,w) = (u(t+1,i,j+1,w)-u(t+1,i,j-1,w))/(2*dV4);
                    uvv4(i,j,w) = (u(t+1,i,j+1,w)-2*u(t+1,i,j,w)+u(t+1,i,j-1,w))/(dV4^2);
                else
...
%                 end
                ...
                if t~=1
                    ...
                    if length(Optimums)>1
                        ...
                        for y=2:length(Optimums)
                            ...
                            CostVal(y-1) = HamToMin(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),phit4(V4(j),Optimums{y}(2)),phit1(V1(i),Optimums{y}(1)),Optimums{y}(3)/FuelMax,t,cwH,Y_hat(w));
                ...
                if t==length(time)
                    ...
                    cwH = cw{t+1,i,j};
                    HAM(i,j) = HamToMin(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),PhiT4(t+1,i,j,w),PhiT1(t+1,i,j,w),PhiF(t+1,i,j,w),t+1,cwH,Y_hat(w));
                    HAM_Plot(t,i,j,w) = HAM(i,j);
                    HAM_Plot_HF(t,i,j,w) = HamToMin_HF(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),PhiT4(t+1,i,j,w),PhiT1(t+1,i,j,w),PhiF(t+1,i,j,w),t+1,cwH,Y_hat(w));
                    ...
                    if t~=1
                        if NonLinCounter == 10000
                            ...
                            while NoSolution ~= 0
                                ...
                                fun = @(x) HamToMin(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),x(2),x(1),x(3),t,cwH,Y_hat(w));
                                ...
                                if abs(x(1)-PhiT1(t,i,j,w))>=0.001 || abs(x(2)-PhiT4(t,i,j,w))>=0.001 || abs(x(3)-PhiF(t,i,j,w))>=0.001
                                    ...
                                    if NoSolution == 10000
                                        ...
                                        disp(['Our solution H = ',num2str(HamToMin(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),PhiT4(t,i,j,w),PhiT1(t,i,j,w),PhiF(t,i,j,w),t,cwH,Y_hat(w)))]);
                                        disp(['MATLAB solution (sum(P)-DW(t,w))=',num2str(PowH1(x(1),V1(i))+PowH4(x(2),V4(j))+PowF(x(3))-DW(t,w))]);
                                        disp(['MATLAB solution H = ',num2str(HamToMin(uv4(i,j,w),uv1(i,j,w),uvv4(i,j,w),uvv1(i,j,w),x(2),x(1),x(3),t,cwH,Y_hat(w)))]);
...
%         set(groot,'defaultfigureposition',[0 0 Screensize(3) Screensize(4)]);
 ...
 %       set(groot,'defaultfigureposition',[0 0 Screensize(3)/2 Screensize(4)/2]);
    ...
    set(0,'CurrentFigure',17);
    title(['Cost Functon (Time = ',num2str(t),')']);
    xlabel('Normalized Wind Power Domain');
    ylabel('Cost');
    grid minor;
    hold on;
    
    plot(Y_hat,squeeze(u(t,sV1*2,sV4*2,:))/NormC,'*')
    plot(-alpha:0.1:alpha,polyval(cw{t,sV1*2,sV4*2}/NormC,-alpha:0.1:alpha));
