...
%     DW2(:,i) = interp1(time,squeeze(DW(:,i)),time2,'spline');
    DW2(:,i) = interp1(time,squeeze(DW(:,i)),time2);
    Dem = interp1(time,D,time2);
end
% === WIND ===>>>
    ...
    for k=1:length(Y_hat)
        ...
        for i=1+Reduction:length(V1)-Reduction
            for j=1+Reduction:length(V4)-Reduction
                ...
                if (i~=1+Reduction) && (j~=1+Reduction) && (i~=length(V1)-Reduction) && (j~=length(V4)-Reduction) && (k~=1+Reduction) && (k~=length(Y_hat)-Reduction) && (t~=length(time))
                    ...
                    if (j==1+Reduction || j==length(V4)-Reduction || k==1+Reduction || k==length(Y_hat)-Reduction) && (i~=1+Reduction) && (i~=length(V1)-Reduction) && (t~=length(time))
                        if IT1(t+1)-FlowMax1*PhiT1(t+1,i,j,k)>=0
                            uv1(i,j,k) = (u(t+1,i+1,j,k)-u(t+1,i,j,k))/(dV1);
                            du1val(t,i,j,k) = 1;
                        else
                    ...
                    if (i==1+Reduction || i==length(V1)-Reduction || k==1+Reduction || k==length(Y_hat)-Reduction) && j~=1+Reduction && j~=length(V4)-Reduction && (t~=length(time))
                        if IT4(t+1)-FlowMax4*PhiT4(t+1,i,j,k)>=0
                            uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j,k))/(dV4);
                            du4val(t,i,j,k) = 1;
                        else
                    ...
                    % dW I
                    if k==1+Reduction && (t~=length(time))
                    ...
                    if k==length(Y_hat)-Reduction && (t~=length(time))
                        if d_pW(t+1)+theta_W(RealTime(t+1))*(pW(t+1)-Y_hat(k-1))>0
                            uw(i,j,k) = (u(t+1,i,j,k)-u(t+1,i,j,k-1))/(dw);
                            duWval(t,i,j,k) = 1;
                        else
                    ...
                    if (i==1+Reduction || i==length(V1)-Reduction || j==1+Reduction || j==length(V4)-Reduction) && k~=1+Reduction && k~=length(Y_hat)-Reduction && (t~=length(time))
                        if d_pW(t+1)+theta_W(RealTime(t+1))*(pW(t+1)-Y_hat(k))>0
                            uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k))/(dw);
                            duWval(t,i,j,k) = 1;
                        else
                    ...
                    if t~=1 && multOP == 0
                        if NonLinCounter == 100
                            ...
                            while NoSolution ~= 0
                                ...
                                ub = ones(1,3);
                                x0 = rand(1,3);
                                options = optimoptions('fmincon','Display','notify','MaxFunctionEvaluations',10000,'FunctionTolerance',1.0000e-07);
                                [x,~,flagFmincon,outputFmincon] = fmincon(fun,x0,A,b,Aeq,beq,lb,ub,@nonlcon,options);

                                if (abs(x(1)-PhiT1(t,i,j,k))>=0.001 || abs(x(2)-PhiT4(t,i,j,k))>=0.001 || abs(x(3)-PhiF(t,i,j,k))>=0.001) && HamToMin(uv4(i,j,k),uv1(i,j,k),uvv4(i,j,k),uvv1(i,j,k),uw(i,j,k),uww(i,j,k),x(2),x(1),x(3),t,Y_hat(k))>HamToMin(uv4(i,j,k),uv1(i,j,k),uvv4(i,j,k),uvv1(i,j,k),uw(i,j,k),uww(i,j,k),PhiT4(t,i,j,k),PhiT1(t,i,j,k),PhiF(t,i,j,k),t,Y_hat(k))
                                    ...
                                    if NoSolution == 10000
                                        ...
                                        error2(E2:E2+2) = [abs(x(1)-PhiT1(t,i,j,k)),abs(x(2)-PhiT4(t,i,j,k)),abs(x(3)-PhiF(t,i,j,k))];
                                        return;
                                    end
                                else
                                    error2(E2:E2+2) = [abs(x(1)-PhiT1(t,i,j,k)),abs(x(2)-PhiT4(t,i,j,k)),abs(x(3)-PhiF(t,i,j,k))];
...
%     hmax = 7;
    ...
    set(0,'CurrentFigure',6);
    vline(time(t),'r',' ');
    title(['Effective Demand (Time = ',num2str(t),')']);
    if t == 1
        saveas(gcf,'DamsAndWind_4','epsc');
    end
    ...
    hold on;
    for k=hmin:hm:hmax
        s0 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(u(t,newStart:end-Reduction,newStart:end-Reduction,k))'/NormC);
        s0.EdgeColor = 'none';
    end
    xlabel('Volume 1');
    ylabel('Volume 4');
    ...
    for k=1:hm:length(Y_hat)
        grad = gradient(squeeze(u(t,newStart:end-Reduction,newStart:end-Reduction,k)));
    ...
    hold on;
    for k=hmin:hm:hmax
        s2 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(Lambda(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s2.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s3 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(PhiT1(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s3.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s4 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(PhiT4(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s4.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s5 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(PhiF(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s5.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s6 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(duWval(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s6.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s7 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(Ph1_Plot(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s7.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s8 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(Ph4_Plot(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s8.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s9 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uv1(newStart:end-Reduction,newStart:end-Reduction,k))'/NormC);
%         s9.EdgeColor = 'none';
    end
    xlabel('Volume 1');
    ylabel('Volume 4');
    zlabel('$Cost/V_1$','interpreter','latex');
    title(['Der. w.r.t. Volume 1 (Time = ',num2str(t),')']);
    xlim([Vmin1+dV1*Reduction Vmax1-dV1*Reduction]);
    ylim([Vmin4+dV4*Reduction Vmax4-dV4*Reduction]);
    zlim auto
    view(140,20);
    ...
    hold on;
    for k=hmin:hm:hmax
        s10 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uv4(newStart:end-Reduction,newStart:end-Reduction,k))'/NormC);
%         s10.EdgeColor = 'none';
    end
    xlabel('Volume 1');
    ylabel('Volume 4');
    zlabel('$Cost/V_4$','interpreter','latex');
    title(['Der. w.r.t. Volume 4 (time=',num2str(t),')']);
    xlim([Vmin1+dV1*Reduction Vmax1-dV1*Reduction]);
    ylim([Vmin4+dV4*Reduction Vmax4-dV4*Reduction]);
    zlim auto
    view(140,20);
    ...
    hold on;
    for k=hmin:hm:hmax
        s11 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(HAM_Plot(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s11.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s12 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(HAM_Plot_HF(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s12.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s13 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(HAM_Plot_W(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s13.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s14 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(du1val(t,newStart:end-Reduction,newStart:end-Reduction,k))');
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s15 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(du4val(t,newStart:end-Reduction,newStart:end-Reduction,k))');
%         s14.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s16 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(duWval(t,newStart:end-Reduction,newStart:end-Reduction,k))');
%         s14.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s17 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uw(newStart:end-Reduction,newStart:end-Reduction,k))'/NormC);
%         s9.EdgeColor = 'none';
    end
    xlabel('Volume 1');
    ylabel('Volume 4');
    zlabel('$Cost/WP$','interpreter','latex');
    title(['Der. w.r.t. Wind Power (Time = ',num2str(t),')']);
    xlim([Vmin1+dV1*Reduction Vmax1-dV1*Reduction]);
    ylim([Vmin4+dV4*Reduction Vmax4-dV4*Reduction]);
    zlim auto
    view(140,20);
    ...
    hold on;
    for k=hmin:hm:hmax
        s18 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uww(newStart:end-Reduction,newStart:end-Reduction,k))'/NormC);
%         s9.EdgeColor = 'none';
    end
    xlabel('Volume 1');
    ylabel('Volume 4');
    zlabel('$Cost/(WP)^2$','interpreter','latex');
    title(['Sec. Der. w.r.t. Wind Power (Time = ',num2str(t),')']);
    xlim([Vmin1+dV1*Reduction Vmax1-dV1*Reduction]);
    ylim([Vmin4+dV4*Reduction Vmax4-dV4*Reduction]);
    zlim auto
    view(140,20);
...
%     end
    ...
    title(['Cost Function (Time = ',num2str(t),')']);
    xlabel('Normalized Wind Power Domain');
    ylabel('Cost');
    grid minor;
    hold on;
    plot(Y_hat,squeeze(u(t,length(V1)/2+0.5,length(V4)/2+0.5,:))/NormP,'-*');
    if SaveFig == 1
    ...
    title(['Derivative w.r.t. Wind Power (Time = ',num2str(t),')']);
    xlabel('Normalized Wind Power Domain');
    ylabel('$Cost/WP$','interpreter','latex');
    grid minor;
    hold on;
    plot(Y_hat,squeeze(uw(length(V1)/2+0.5,length(V4)/2+0.5,:))/NormP,'-*');
    if SaveFig == 1
        saveas(gcf,['Cost_Wind_',num2str(t)],'epsc');
    elseif SaveFig ~= 0
        disp('Choose SaveFig between 0 or 1');
    ...
    title(['Second Derivative w.r.t. Wind Power (Time = ',num2str(t),')']);
    xlabel('Normalized Wind Power Domain');
    ylabel('$Cost/(WP)^2$','interpreter','latex');
    grid minor;
    hold on;
    plot(Y_hat,squeeze(uww(length(V1)/2+0.5,length(V4)/2+0.5,:))/NormP,'-*');
    if SaveFig == 1
        saveas(gcf,['Cost_Wind_',num2str(t)],'epsc');
    elseif SaveFig ~= 0
        disp('Choose SaveFig between 0 or 1');
...
if SaveU == 1
    save(['u_V1_',num2str(length(V1)-1),'_V4_',num2str(length(V4)-1),'_W_',num2str(length(Y_hat)-1),'_T_',num2str(NT3),'.mat',],'u');
elseif SaveU ~= 0
