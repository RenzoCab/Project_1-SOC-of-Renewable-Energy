...
for i=1:length(Y_hat)
    for j=1:length(SigW2)
        Y(j,i) = pW(j)+Y_hat(i)*SigW2(j)/2;
        DW(j,i) = max(0,D2(j)-Y(j,i)*Gk);
    end
end
cw{length(time2),length(V1),length(V4)} = {};
% === WIND ===>>>
...
u = zeros(length(time2),length(V1),length(V4),length(Y_hat)); % 3 wind lines.
Ph4 = zeros(length(time2),length(V1),length(V4));
Ph1 = zeros(length(time2),length(V1),length(V4));
Pf = zeros(length(time2),length(V1),length(V4));
PhiT4 = zeros(length(time2),length(V1),length(V4),length(Y_hat));
PhiT1 = zeros(length(time2),length(V1),length(V4),length(Y_hat));
PhiF = zeros(length(time2),length(V1),length(V4),length(Y_hat));
Ph4_Plot = zeros(length(time2),length(V1),length(V4),length(Y_hat));
Ph1_Plot = zeros(length(time2),length(V1),length(V4),length(Y_hat));
Pf_Plot = zeros(length(time2),length(V1),length(V4),length(Y_hat));
HAM_Plot = zeros(length(time2),length(V1),length(V4),length(Y_hat));
HAM_Plot_HF = zeros(length(time2),length(V1),length(V4),length(Y_hat));
HAM_Plot_W = zeros(length(time2),length(V1),length(V4),length(Y_hat));
...
HamToMin = @(uv4,uv1,phit4,phit1,phif,t,cw,Y_hat) TMax*(hatKh4(uv4)*phit4 + hatKh1(uv1)*phit1 + hatKf*phif + IT4(t)*uv4/VolMax4 + IT1(t)*uv1/VolMax1) + (-Y_hat*(theta_W(t)+d_SigW2(t)/SigW2(t)))*(6*cw(1)*Y_hat^5+5*cw(2)*Y_hat^4+4*cw(3)*Y_hat^3+3*cw(4)*Y_hat^2+2*cw(5)*Y_hat+cw(6)) + (4*theta_W(t)*alpha_W/(SigW2(t)^2))*(Y_hat*SigW2(t)/2+pW(t))*(1-Y_hat*SigW2(t)/2-pW(t))*(6*5*cw(1)*Y_hat^4+5*4*cw(2)*Y_hat^3+4*3*cw(3)*Y_hat^2+3*2*cw(4)*Y_hat+2*cw(5));
...
HamToMin_W = @(uv4,uv1,phit4,phit1,phif,t,cw,Y_hat) (-Y_hat*(theta_W(t)+d_SigW2(t)/SigW2(t)))*(6*cw(1)*Y_hat^5+5*cw(2)*Y_hat^4+4*cw(3)*Y_hat^3+3*cw(4)*Y_hat^2+2*cw(5)*Y_hat+cw(6)) + (4*theta_W(t)*alpha_W/(SigW2(t)^2))*(Y_hat*SigW2(t)/2+pW(t))*(1-Y_hat*SigW2(t)/2-pW(t))*(6*5*cw(1)*Y_hat^4+5*4*cw(2)*Y_hat^3+4*3*cw(3)*Y_hat^2+3*2*cw(4)*Y_hat+2*cw(5));
...
for t=length(time2):-1:1
    
    disp(['Complete: ',num2str((length(time2)-t)*100/length(time2)),'%.']);
    uv4 = zeros(length(V1),length(V4),length(Y_hat));
    uv1 = zeros(length(V1),length(V4),length(Y_hat));
    ...
    for w=1:length(Y_hat)
        ...
        for i=1+Reduction:length(V1)-Reduction
            for j=1+Reduction:length(V4)-Reduction
                ...
                if (i~=1+Reduction) && (j~=1+Reduction) && (i~=length(V1)-Reduction) && (j~=length(V4)-Reduction) && (t~=length(time2))
                    uv1(i,j,w) = (u(t+1,i+1,j,w)-u(t+1,i-1,j,w))/(2*dV1);
                    uv4(i,j,w) = (u(t+1,i,j+1,w)-u(t+1,i,j-1,w))/(2*dV4);
                else

                    if i==1+Reduction && (t~=length(time2))
                        uv1(i,j,w) = (u(t+1,i+1,j,w)-u(t+1,i,j,w))/(dV1);
                    end
                    if i==length(V1)-Reduction && (t~=length(time2))
                        uv1(i,j,w) = (u(t+1,i,j,w)-u(t+1,i-1,j,w))/(dV1);
                    end
                    if j==1+Reduction && (t~=length(time2))
                        uv4(i,j,w) = (u(t+1,i,j+1,w)-u(t+1,i,j,w))/(dV4);
                    end
                    if j==length(V4)-Reduction && (t~=length(time2))
                        uv4(i,j,w) = (u(t+1,i,j,w)-u(t+1,i,j-1,w))/(dV4);
                    end
                    if (j==1+Reduction || j==length(V4)-Reduction) && (i~=1+Reduction) && (i~=length(V1)-Reduction) && (t~=length(time2))
                        uv1(i,j,w) = (u(t+1,i+1,j,w)-u(t+1,i-1,j,w))/(2*dV1);
                    end
                    if (i==1+Reduction || i==length(V1)-Reduction) && j~=1+Reduction && j~=length(V4)-Reduction && (t~=length(time2))
                        uv4(i,j,w) = (u(t+1,i,j+1,w)-u(t+1,i,j-1,w))/(2*dV4);
                    end
                ...
                if CinZ1(V1(i),uv1(i,j,w))<0 && CinZ4(V4(j),uv4(i,j,w))<0 && Ph_star1(V1(i),uv1(i,j,w))>0 && Ph_star4(V4(j),uv4(i,j,w))>0
                    ...
                    if DW(t,w)>olPh1(V1(i)) && DW(t,w)<=olPh4(V4(j))
                        ...
                        elseif Ph_star1(V1(i),uv1(i,j,w))+Ph_star4(V4(j),uv4(i,j,w))>DW(t,w) && Ph_star4(V4(j),uv4(i,j,w))>DW(t,w)-olPh1(V1(i))
                            if not((dCostPh4(uv1(i,j,w),uv4(i,j,w),V1(i),V4(j),DW(t,w),DW(t,w)-olPh1(V1(i)))>0 && dCostPh4(uv1(i,j,w),uv4(i,j,w),V1(i),V4(j),DW(t,w),DW(t,w))<0))% && Ph_StarStar(t,uv1(i,j,w),uv4(i,j,w),V1(i),V4(j))>0
                    ...
                    if DW(t,w)>olPh1(V1(i)) && DW(t,w)>olPh4(V4(j)) && DW(t,w)<=olPh1(V1(i))+olPh4(V4(j))
                        ...
                        elseif Ph_star4(V4(j),uv4(i,j,w))>olPh4(V4(j)) && Ph_star1(V1(i),uv1(i,j,w))<=DW(t,w)-olPh4(V4(j))
                            ...
                            Pf(t,i,j) = DW(t,w) - Ph1(t,i,j) - Ph4(t,i,j);
                        elseif Ph_star1(V1(i),uv1(i,j,w))+Ph_star4(V4(j),uv4(i,j,w))>DW(t,w) && Ph_star4(V4(j),uv4(i,j,w))>DW(t,w)-olPh1(V1(i)) && Ph_star1(V1(i),uv1(i,j,w))>DW(t,w)-olPh4(V4(j))
                            if not((dCostPh4(uv1(i,j,w),uv4(i,j,w),V1(i),V4(j),DW(t,w),DW(t,w)-olPh1(V1(i)))>0 && dCostPh4(uv1(i,j,w),uv4(i,j,w),V1(i),V4(j),DW(t,w),olPh4(V4(j)))<0))% && Ph_StarStar(t,uv1(i,j,w),uv4(i,j,w),V1(i),V4(j))>0
                    ...
                    disp(['Time: ',num2str(100*(length(time2)-t)/length(time2)),'%']);
...
%     legend('Low V1 Low V4','Low V1 Low V4','High V1 Low V4','High V1 Low V4','Low V1 High V4','Low V1 High V4','High V1 High V4','High V1 High V4');
    ...
    C = Y_hat*SigW2(t)/2+ExW2(t);
    D3 = [minA:(min(C)-minA)/10:min(C),C(2:end-1),max(C):(maxA-max(C))/10:maxA];
    F = 2*(D3-ExW2(t))/SigW2(t);
    ...
    for i=1:4
        ...
        switch i
            case 1
                ...
                plot(D3(11:17),squeeze(u(t,sV1,sV4,:)),'*');
                plot(D3,Sol);
                xlim([minA,maxA]);
            ...
            case 2
                ...
                plot(D3(11:17),squeeze(u(t,sV1*4,sV4,:)),'*');
                plot(D3,Sol);
                xlim([minA,maxA]);
            ...
            case 3
                ...
                plot(D3(11:17),squeeze(u(t,sV1,sV4*4,:)),'*');
                plot(D3,Sol);
                xlim([minA,maxA]);
            ...
            case 4
                ...
                plot(D3(11:17),squeeze(u(t,sV1*4,sV4*4,:)),'*');
                plot(D3,Sol);
                xlim([minA,maxA]);
