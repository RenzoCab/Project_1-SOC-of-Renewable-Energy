...
%                     uv1(i,j,k) = (u(t+1,i+1,j,k)-u(t+1,i-1,j,k))/(2*dV1);
%                     uvv1(i,j,k) = (u(t+1,i+1,j,k)-2*u(t+1,i,j,k)+u(t+1,i-1,j,k))/(dV1^2);
%                     uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j-1,k))/(2*dV4);
%                     uvv4(i,j,k) = (u(t+1,i,j+1,k)-2*u(t+1,i,j,k)+u(t+1,i,j-1,k))/(dV4^2);
%                     uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k-1))/(2*dw);
...
%                     uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k))/(dw);
                    ...
                    if IT1(t)-FlowMax1*PhiT1(t+1,i,j,k)>=0
                        uv1(i,j,k) = (u(t+1,i+1,j,k)-u(t+1,i,j,k))/(dV1);
                        du1val(t,i,j,k) = 1;
                    else
                    ...
                    if IT4(t)-FlowMax4*PhiT4(t+1,i,j,k)>=0
                        uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j,k))/(dV4);
                        du4val(t,i,j,k) = 1;
                    else
                    ...
                    if d_pW(t)+theta_W(RealTime(t))*(pW(t)-Y_hat(k))>0
                        uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k))/(dw);
                        duWval(t,i,j,k) = 1;
                    else
...
%                     uv1(i,j,k) = (u(t+1,i,j,k)-u(t+1,i-1,j,k))/(dV1); %this works
%                     uv4(i,j,k) = (u(t+1,i,j,k)-u(t+1,i,j-1,k))/(dV4);
%                     uw(i,j,k) = (u(t+1,i,j,k)-u(t+1,i,j,k-1))/(dw);
                else
...
%                         uv1(i,j,k) = (u(t+1,i+1,j,k)-u(t+1,i,j,k))/(dV1);
                        if IT1(t)-FlowMax1*PhiT1(t+1,i,j,k)>=0
                            ...
                            u0 = 2*u(t+1,i,j,k)-u(t+1,i+1,j,k);
                            uv1(i,j,k) = (u(t+1,i,j,k)-u0)/(dV1);
                            du1val(t,i,j,k) = -1;
                        end
                    end
...
%                         uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j,k))/(dV4);
                        if IT4(t)-FlowMax4*PhiT4(t+1,i,j,k)>=0
                            ...
                            u0 = 2*u(t+1,i,j,k)-u(t+1,i,j+1,k);
                            uv4(i,j,k) = (u(t+1,i,j,k)-u0)/(dV4);
                            du4val(t,i,j,k) = -1;
                        end
                    end
...
%                         uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j-1,k))/(2*dV4);
%                         uv4(i,j,k) = (u(t+1,i,j,k)-u(t+1,i,j-1,k))/(dV4);%this works
                        if IT4(t)-FlowMax4*PhiT4(t+1,i,j,k)>=0
                            uv4(i,j,k) = (u(t+1,i,j+1,k)-u(t+1,i,j,k))/(dV4);
                            du4val(t,i,j,k) = 1;
                        else
...
%                         uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k))/(dw);
                        if d_pW(t)+theta_W(RealTime(t))*(pW(t)-Y_hat(k))>0
                            ...
                            u0 = 2*u(t+1,i,j,k)-u(t+1,i,j,k+1);
                            uw(i,j,k) = (u(t+1,i,j,k)-u0)/(dw);
                            duWval(t,i,j,k) = -1;
                        end
                    end
...
%                         uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k-1))/(2*dw);
%                         uw(i,j,k) = (u(t+1,i,j,k)-u(t+1,i,j,k-1))/(dw);%this works
                        if d_pW(t)+theta_W(RealTime(t))*(pW(t)-Y_hat(k))>0
                            uw(i,j,k) = (u(t+1,i,j,k+1)-u(t+1,i,j,k))/(dw);
                            duWval(t,i,j,k) = 1;
                        else
...
%     hmax = length(Y_hat);
    ...
    hold on;
    for k=hmin:hm:hmax
        s0 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(u(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s0.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
...
%     box;
    ...
    hold on;
    for k=hmin:hm:hmax
%         s6 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(Pf_Plot(t,newStart:end-Reduction,newStart:end-Reduction,k))');
...
%     title(['Fuel Power (time=',num2str(t),')']);
    ...
    hold on;
    for k=hmin:hm:hmax
        s9 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uv1(newStart:end-Reduction,newStart:end-Reduction,k))');
%         s9.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
    ...
    hold on;
    for k=hmin:hm:hmax
        s10 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(uv4(newStart:end-Reduction,newStart:end-Reduction,k))');
%         s10.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
...
%         s12 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(HAM_Plot_HF(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s12 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(du4val(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s12.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
...
%         s13 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(HAM_Plot_W(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s13 = surf(v1(newStart:end-Reduction,newStart:end-Reduction),v4(newStart:end-Reduction,newStart:end-Reduction),squeeze(du1val(t,newStart:end-Reduction,newStart:end-Reduction,k))');
        s13.EdgeColor = 'none';
    end
    xlabel('V1');
    ylabel('V4');
...
if SaveU == 1
    save(['u_',num2str(length(V1)),'x',num2str(length(V4)),'_T',num2str(NT2),'.mat',],'u');
elseif SaveU ~= 0
