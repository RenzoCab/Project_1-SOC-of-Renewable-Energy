...
% Aun hay cosas mal, no estoy veriicando monotonicidad y ademas cuando
% aplico la BC que seria solo copiar la derivada, la estoy aplicando antes
% de calcularla, lo que esta mal.
...
for t=length(time):-1:1
    ...
    for i=1:length(V)
        ...
        elseif (i==1) && (t~=length(time))
            uv(i) = uv(i+1);
        elseif (i==length(V)) && (t~=length(time))
            uv(i) = uv(i-1);
        end
