...
function [] = realBalancePlot(dateString)
    ...
    Others = {'Biomass Power','Solar Power','Wind Power'};
    ...
    plotArea = area(time,[data_1_1(:,8),data_1_1(:,6),data_1_1(:,5),data_1_1(:,1),...
        data_1_1(:,2),data_1_1(:,3),data_1_1(:,4),Motores,PTA8,PTB,CTR]);
    hold on;
    P = plot(time,data_1_1(:,12)+data_1_6(:,1)+data_1_6(:,2)+data_1_6(:,3)...
        -data_1_6(:,4)-data_1_6(:,5)-data_1_6(:,6),'m');
    P.LineWidth = 1;
    grid minor;
    legend(Others{:},Dams{:},Termicas{:},'Demand + EXP - IMP','location','eastoutside');
    plotArea(1).FaceColor = [1 .5 0]; % Biomasa.
    plotArea(2).FaceColor = [1 1 .5]; % Solar.
    plotArea(3).FaceColor = [0 1 0]; % Wind.
    plotArea(4).FaceColor = [.5 .5 1]; % SG.
    plotArea(5).FaceColor = [.5 .8 1]; % Bonete.
    plotArea(6).FaceColor = [.2 .2 1]; % Baygorria.
    plotArea(7).FaceColor = [.8 .8 1]; % Palmar.
    plotArea(8).FaceColor = [1 .6 .6]; % T1 (Motores).
    plotArea(9).FaceColor = [1 .3 .5]; % T2 (PTA8).
    plotArea(10).FaceColor = [145/255 0 211/255;]; % T3 (PTB).
    plotArea(11).FaceColor = [1 0 0]; % T4 T4 (CTR).
    ylabel('Power (MW)');
    xlabel('Time');
    ...
    saveas(gcf,[pwd '/Historical/realBalancePlot/all_',dateString],'epsc');
