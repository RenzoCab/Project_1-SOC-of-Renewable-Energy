...
% 4 - Historical path (I will remove this soon).
% 5 - 
...
%     [vplotA_1,vplot1_A] = meshgrid(A,V1); % For plotting surfaces.
    ...
    olCA  = @(uA)          -K_A*uA;
    olCV2 = @(uV2,t)       MaxTot2*(Lambda21(t) + uV2/VolMax2 - CH2);
    olCV3 = @(uV3,t)       MaxTot3*(Lambda32(t) + uV3/VolMax3 - CH3);
...
%     k = @(t,V2,V3) [k1(t),k2(V2,V3),k3(V2,V3)];
    ...
    if WhatToDo == 2 || WhatToDo == 3 || WhatToDo == 4 || WhatToDo == 5 || WhatToDo == 6 || WhatToDo == 7
        ...
        if WhatToDo == 4
            load([pwd '/','Historical/Energy_Controls_',Day,'_2019.mat']);
        end
        ...
        if WhatToDo ~= 5
            ...
            for t = 1:length(time)
                ...
                % problem.
                realInstantCost(t) = sum(dt*TMax*Controls_OP(t,:).*...
                    [FlowMax1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))*CH1,...
                    MaxSpill1(Vol_OP_Real_1(t),Vol_OP_Real_2(t))*CH1,...
                    FlowMax2(Vol_OP_Real_2(t),Vol_OP_Real_3(t))*CH1,...
                    MaxSpill2(Vol_OP_Real_2(t),Vol_OP_Real_3(t))*CH1,...
                    ...
                    MaxV2*CH2,...
                    MaxV3*CH3]) + ...
                    dt*TMax*(IT1(t)*CH1+IT2(t)*CH2+IT3(t)*CH3+IT4(t)*CH4);
                ...
                if t == 1
                    accumRealInstantCost(t) = realInstantCost(t);
                end
                if t > 1
                    accumRealInstantCost(t) = accumRealInstantCost(t-1) + realInstantCost(t);
            ...
            for t = 1:length(time)-1
                futureCostOP(t) = sum(instantTotalCost(t:end));
                futureRealCostOP(t) = sum(realInstantCost(t:end));
   ...
   %                 legend('Biomass Power','Solar Power','Wind Power','Salto Grande (Hydro)','Bonete (Hydro)','Baygorria (Hydro)','Palmar (Hydro)',...
    %                    'Motores Batlle (Fossil)','PTA (Fossil)','PTB (Fossil)','CTR (Fossil)','Failure (Fossil)','Demand','Demand + Exp.','location','eastoutside');
                ...
                if WhatToDo == 7 || WhatToDo == 4 || WhatToDo == 3 || WhatToDo == 2 || WhatToDo == 6
                    P = plot(time,ones(1,length(time))*Final_Cost); P.LineWidth = 1;
                else
                ...
                hold on;
                plot(time,instantTotalCost/NormCost);
                ...
                if WhatToDo == 4
                    sets = [102,103,104,110,107,106,108,127,113,121,112,118,116,117,125,126,123,124];
                    for m = 1:length(sets)
                        i = sets(m);
                        set(0,'CurrentFigure',i);
                        saveas(gcf,[pwd '/',['Simulations/Simulation_',name],'/R_',num2str(i)],'epsc');
                    end
                end
