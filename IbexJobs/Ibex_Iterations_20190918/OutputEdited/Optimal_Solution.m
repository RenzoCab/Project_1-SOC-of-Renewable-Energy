...
% Website: None.
% June 2019; Last revision: 23/09/2019.
...
%     k = @(t,V2,V3) [k1(t),k2(V2,V3),k3(V2,V3)];
    ...
    if WhatToDo == 2 || WhatToDo == 3 || WhatToDo == 4 || WhatToDo == 5 || WhatToDo == 6 || WhatToDo == 7
        ...
        load([pwd '/','Simulations/Simulation_',Day,'.mat']);
   ...
   %                 legend('Biomass Power','Solar Power','Wind Power','Salto Grande (Hydro)','Bonete (Hydro)','Baygorria (Hydro)','Palmar (Hydro)',...
    %                    'Motores Batlle (Fossil)','PTA (Fossil)','PTB (Fossil)','CTR (Fossil)','Failure (Fossil)','Demand','Demand + Exp.','location','eastoutside');
                ...
                xlim([0 max(time)]);
                set(gca,'Box','on');
                set(findall(gcf,'-property','FontSize'),'FontSize',fontsize+4);
...
%                     'Zero reference','location','southeast');
                legend('Real cost Optial Path','southeast');
                ...
                hold on; grid on; grid minor;
                title('Fossil Fuel Stations Controls');
                xlabel('Time');
                ylabel('Control');
                for q = 8:12
                    P = plot(time,Controls_OP(:,q));
                ...
                if Format_Long == 0
                	...
                	set(0,'CurrentFigure',120); clf(120); set(gcf,'Position',[10 10 750*2 300*2],'name','Power Balance Controllable');
                end
                grid minor; hold on;
                if UseBattery == 1
