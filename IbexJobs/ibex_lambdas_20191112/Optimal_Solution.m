...
%                     accumRealInstantCost(t) = accumRealInstantCost(t-1) + realInstantCost(t); ccc
   ...
   %                 legend('Biomass Power','Solar Power','Wind Power','Salto Grande (Hydro)','Bonete (Hydro)','Baygorria (Hydro)','Palmar (Hydro)',...
    %                    'Motores Batlle (Fossil)','PTA (Fossil)','PTB (Fossil)','CTR (Fossil)','Failure (Fossil)','Demand','Demand + Exp.','location','eastoutside');
                ...
                P = plot(time,uV2plot/NormCost); P.LineWidth = 1;
                P = plot(time,-CH2*ones(1,length(time))); P.LineWidth = 1;
                P = plot(time,Lambda21(1:length(time))); P.LineWidth = 1;
                grid minor;
                title('Costs over Historical Path (Virtual Controls)');
                legend('Instant Cost Virtual Baygorria','Partial derivative w.r.t. V2',...
                    'Minus CH2','Lambda_21');
                set(gca,'Box','on');
                ...
                P = plot(time,uV3plot/NormCost); P.LineWidth = 1;
                P = plot(time,-CH3*ones(1,length(time))); P.LineWidth = 1;
                P = plot(time,Lambda32(1:length(time))); P.LineWidth = 1;
                grid minor;
                title('Costs over Historical Path (Virtual Controls)');
                legend('Instant Cost Virtual Palmar','Partial derivative w.r.t. V3',...
                    'Minus CH3','Lambda_32');
                set(gca,'Box','on');
